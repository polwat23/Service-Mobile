...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	...
	$fetchDataOfficer = $conmysql->prepare("SELECT type FROM webcoopboardofdirectors WHERE is_use <>'-9' AND emp_type ='1'");     
	...
	if($typeData =="0"){
		$fetchBoadCoop = $conmysql->prepare("
										...
										LEFT JOIN webcoopbranch branch ON branch.branch_id =b.branch_id 
										WHERE b.is_use <> '-9'AND b.emp_type ='1' AND post.is_use <>'9' AND (post.level = '1' OR post.level='2')
										");
	}else{
		$fetchBoadCoop = $conmysql->prepare("
										SELECT
