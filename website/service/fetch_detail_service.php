...
if($lib->checkCompleteArgument(['unique_id','service_id'],$dataComing)){
	...
	$fetchDeposit = $conmysql->prepare("
										...
										");
	$fetchDeposit->execute(['service_id' => $dataComing['service_id']]);
	while($rowNewsWebCoop = $fetchDeposit->fetch(PDO::FETCH_ASSOC)){
		...
		$arrNewsWebCoop["GROUPSERVICE_ID"] = $rowNewsWebCoop["type"];
