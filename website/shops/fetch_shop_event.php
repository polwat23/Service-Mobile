...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	...
	while($rowShopEvent = $fetchNewsWebCoop->fetch(PDO::FETCH_ASSOC)){
		...
		while($rowFile = $fetchNewsFileCoop->fetch(PDO::FETCH_ASSOC)){
			...
			$arrNewsFile = array();	
			$arrNewsFile["uid"] = $rowFile["id_webcoopfile"];
			$arrNewsFile["path"] = $rowFile["file_patch"];
			$arrNewsFile["thumbUrl"] = $rowFile["file_url"];
