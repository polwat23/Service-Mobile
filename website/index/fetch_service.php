...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
		...
		$arrayGroup = array();
		$fetchServiceGroup = $conmysql->prepare("SELECT DISTINCT type FROM webcoopservice");
		$fetchServiceGroup->execute();
		while($rowNewsWebCoop = $fetchServiceGroup->fetch(PDO::FETCH_ASSOC)){
		
		$arrNewsWebCoop = [];
		$arrNewsWebCoop["TTTTT"] = $rowNewsWebCoop["type"];
		...
		$fetchDeposit = $conmysql->prepare("
												...
												FROM
													webcoopservice
												WHERE
													is_use = '1' AND type = :type
												ORDER BY
													create_date
												DESC
												LIMIT 1
												");
		$fetchDeposit->execute(['type' => $rowNewsWebCoop["type"]]);
