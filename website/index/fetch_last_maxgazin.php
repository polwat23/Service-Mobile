...
require_once('../autoload.php');
	...
	$arrayGroup = array();
		...
		$fetchMaxgazin = $conmysql->prepare("SELECT
											...
											FROM
												webcoopmaxgazin mz
											LEFT JOIN webcoopgallary gall ON
												mz.id_gallery = gall.id_gallery
											WHERE
												mz.is_use = '1'
											ORDER BY
												mz.update_date
		...
		while($rowMaxgazin = $fetchMaxgazin->fetch(PDO::FETCH_ASSOC)){
			...
			$arrNewsWebCoop["UPDATE_DATE"] = $rowMaxgazin["update_date"];
			...
			$arrNewsWebCoop["UPDATE_DATE_DATE_FORMAT"] = $lib->convertdate($rowMaxgazin["update_date"],'d m Y',true); 
