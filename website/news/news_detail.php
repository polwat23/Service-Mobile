...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
		...
		$fetchNewsWebCoop = $conmysql->prepare("SELECT
										...
										FROM
											webcoopnews
										WHERE
											id_webcoopnews = :news_id AND is_use ='1'");
		...
		while($rowNewsWebCoop = $fetchNewsWebCoop->fetch(PDO::FETCH_ASSOC)){
			...
			$arrNewsWebCoop["NEWS_DETAIL_SHORT"] = $lib->text_limit($rowNewsWebCoop["news_detail"],480);
			...
			$arrNewsWebCoop["NEW_FILE_PATC"] = $filePath;
			$arrNewsWebCoop["NEW_FILE_NAME"] = $fileName;
