...
if($lineLib->checkBindAccount($user_id)){
	...
	$arrGroupBNF = array();
	$getBeneficiary = $conmssql->prepare("SELECT mg.GAIN_NAME,mg.GAIN_SURNAME,mg.GAIN_ADDRESS as GAIN_ADDR,mg.REMARK,mc.GAIN_CONCERN,mg.GAIN_PERCENT
	...
	if(sizeof($arrGroupBNF)>0){
		...
		foreach($arrGroupBNF as $rowBeneficiaryData){
			...
			$beneficiaryData["contents"]["contents"][$indexBeneficiary]["body"]["contents"][0]["contents"][0]["color"] = "#1885C3";
			...
			$beneficiaryData["contents"]["contents"][$indexBeneficiary]["body"]["contents"][0]["contents"][2]["contents"][1]["color"] = "#1885C3";
