...
$header = array();
$deptslip_no = ($rowReceipt["DEPTSLIP_NO"]??'-');
$getPaymentDetail = $conmssql->prepare("SELECT S.DEPTSLIP_NO,M.MEMBER_NO,M.DEPTACCOUNT_NO,P.PRENAME_DESC,
										...
										WHERE S.DEPTSLIP_NO = :deptslip_no AND ITEM_STATUS=1 AND M.MEMBER_NO = :member_no
...
$header["deptslip_no"] = $deptslip_no;
$arrayPDF = GenerateReport($arrGroupDetail,$header,$lib);
if($arrayPDF["RESULT"]){
	$url_report = $config["URL_SERVICE"].$arrayPDF["PATH"];
}else{
