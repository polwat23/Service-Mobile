...
if($lineLib->checkBindAccount($user_id)){
	...
	$arrTypeAllow = array();
	$arrDeptTypDesc = array();
	$getTypeAllowShow = $conmysql->prepare("SELECT dept_type_code FROM gcconstantaccountdept WHERE allow_showdetail = '1'");
	$getTypeAllowShow->execute();
	while($rowTypeAllow = $getTypeAllowShow->fetch(PDO::FETCH_ASSOC)){
		$arrTypeAllow[] = "'".$rowTypeAllow["dept_type_code"]."'";
	}
	$fetchDepositType =  $conoracle->prepare("SELECT depttype_code,depttype_desc From DPDEPTTYPE where depttype_desc not in('เงินฝากออมทรัพย์พิเศษ 37 ปี (สมทบ)')");
	$fetchDepositType->execute();
	while($rowDeptTypeDesc = $fetchDepositType->fetch(PDO::FETCH_ASSOC)){
		$arrDeptTypDesc[$rowDeptTypeDesc["DEPTTYPE_CODE"]] = $rowDeptTypeDesc["DEPTTYPE_DESC"];
	}

	$fetchDeposit = $conoracle->prepare("SELECT DISTINCT  DEPTTYPE_CODE
										FROM dpdeptmaster 
										WHERE   DEPTTYPE_CODE IN(".implode(',',$arrTypeAllow).") 
											AND member_no = :member_no  
											AND deptclose_status <> 1 ");
	$fetchDeposit->execute([
			':member_no' => $member_no 
	]);
	$getSumdepositAll = $conoracle->prepare("SELECT SUM(prncbal) as SUM_BALANCE FROM dpdeptmaster WHERE member_no = :member_no and deptclose_status <> 1");
	$getSumdepositAll->execute([':member_no' => $member_no]);
	$rowSumAllbalance = $getSumdepositAll->fetch(PDO::FETCH_ASSOC);
	
	$getTotalAllAccount = $conoracle->prepare("SELECT COUNT(deptaccount_no) as TOTALACCOUNT FROM dpdeptmaster WHERE member_no = :member_no and deptclose_status <> 1");
	...
	while($rowDeposit = $fetchDeposit->fetch(PDO::FETCH_ASSOC)){
		$getSumDeposit =  $conoracle->prepare("SELECT SUM(prncbal) as SUM_BALANCE FROM dpdeptmaster WHERE member_no = :member_no  and DEPTTYPE_CODE = :depttype_code  and deptclose_status <> 1");
		$getSumDeposit->execute([
			':member_no' => $member_no,
			':depttype_code' => $rowDeposit["DEPTTYPE_CODE"]
		]);
		$arrSumDeposit = $getSumDeposit->fetch(PDO::FETCH_ASSOC);
		$getTotalAccount = $conoracle->prepare("SELECT COUNT(deptaccount_no) as TOTALACCOUNT FROM dpdeptmaster WHERE member_no = :member_no and DEPTTYPE_CODE = :depttype_code and deptclose_status <> 1");
		...
		$arrDepositData["DEPTTYPE_DESC"] = $arrDeptTypDesc[$rowDeposit["DEPTTYPE_CODE"]]; $arrDeptTypDesc[$rowUserlogin["member_no"]];
	...
	if(sizeof($groupDepositData) > 0){
		...
		foreach($groupDepositData as $deposit){
			...
			$depositData["contents"]["contents"][$indexDeposit]["footer"]["contents"][0]["action"]["text"] = "ประเภทเงินฝาก:".($deposit["DEPTTYPE_CODE"]??'-');
