...
if($lineLib->checkBindAccount($user_id)){
	...
	$getSumdepositData = $conmssql->prepare("SELECT SUM(prncbal) as SUM_BALANCE FROM dpdeptmaster WHERE member_no = :member_no and deptclose_status <> 1");
	$getSumdepositData->execute([':member_no' => $member_no]);
	$rowSumbalance = $getSumdepositData->fetch(PDO::FETCH_ASSOC);
	$getTotalAccount = $conmssql->prepare("SELECT COUNT(deptaccount_no) as TOTALACCOUNT FROM dpdeptmaster WHERE member_no = :member_no and deptclose_status <> 1;");
	...
	$datas = [];
	$datas["type"] = "flex";
	$datas["altText"] = $groupDeposit[0]["TYPE_ACCOUNT"];
	...
	if(sizeof($depositTypeGroup)>0){
		...
		$datas["contents"]["contents"][0]["type"] = "text";
		$datas["contents"]["contents"][0]["text"]["type"] = "ไม่พบข้อมูล";
		$datas["contents"]["contents"][0]["text"]["margin"] = "md";
	}
