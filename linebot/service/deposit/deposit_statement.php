...
if($lineLib->checkBindAccount($user_id)){
	...
	if(sizeof($arrSlipTrans) > 0){
		$fetchStmSeqDept = $conmssql->prepare("SELECT DPSTM_NO FROM dpdeptslip WHERE (deptslip_no IN('".implode("','",$arrSlipTrans)."') OR refer_slipno IN('".implode("','",$arrSlipTrans)."')) and deptaccount_no = :deptacc_no");
	...
	if(sizeof($arrSlipStm) > 0){
		$getStatement = $conmssql->prepare("SELECT TOP ".$limit." dit.DEPTITEMTYPE_DESC AS TYPE_TRAN,dit.SIGN_FLAG,dsm.SEQ_NO,
	...
	$headerData = $conmssql->prepare("SELECT dp.DEPTTYPE_CODE,dt.DEPTTYPE_DESC,dp.DEPTACCOUNT_NO,dp.DEPTACCOUNT_NAME,dp.prncbal as BALANCE,
