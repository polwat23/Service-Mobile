...
if($lineLib->checkBindAccount($user_id)){
	...
	$getWhocollu = $conmssql->prepare("SELECT lnm.principal_balance as PRNBAL,lnm.loancontract_no as LOANCONTRACT_NO,IsNull(lnm.loanapprove_amt,0) as APPROVE_AMT,lt.LOANTYPE_DESC as TYPE_DESC
										...
										GROUP BY lnm.loancontract_no,IsNull(lnm.loanapprove_amt,0),lt.LOANTYPE_DESC,lnm.principal_balance");
					...
					if(sizeof($guarantee["ASSET"]) > 0){
						foreach($guarantee["ASSET"] as $asset){
							...
							if($guarantee["LOANCOLLTYPE_CODE"] == "01"){
								...
								$whoColyouData["contents"]["body"]["contents"][$indexLoan]["contents"][$indexGuarantee]["contents"][$indexAsset]["contents"][1]["color"] = "#0938A4";
								$whoColyouData["contents"]["body"]["contents"][$indexLoan]["contents"][$indexGuarantee]["contents"][$indexAsset]["contents"][2]["text"] = ($asset["DEPT_AMT"]??"0.00"). "บาท";
								$whoColyouData["contents"]["body"]["contents"][$indexLoan]["contents"][$indexGuarantee]["contents"][$indexAsset]["contents"][2]["color"] = "#35B84B";
