...
if($lineLib->checkBindAccount($user_id)){
	...
	$getContract = $conoracle->prepare("SELECT lt.LOANTYPE_DESC AS LOAN_TYPE,RTRIM(ln.LOANCONTRACT_NO) as LOANCONTRACT_NO,ln.principal_balance as LOAN_BALANCE,
	...
	$old_seq_no = "and lsm.SEQ_NO < 999999";
	...
	$getStatement = $conoracle->prepare("SELECT * FROM (SELECT lit.LOANITEMTYPE_DESC AS TYPE_DESC,lsm.operate_date,lsm.principal_payment as PRN_PAYMENT,lsm.SEQ_NO,
											...
											ORDER BY lsm.SEQ_NO DESC) WHERE rownum <= ".$limit." ");
	...
	if(sizeof($arrAllLoan)>0){
		...
		$loanStatement["altText"] = "ข้อมูลเงินกู้ ".$contract_no;
		...
		//$arrPostData["replyToken"] = $arrayGroupSTM; 
	}
}else{
