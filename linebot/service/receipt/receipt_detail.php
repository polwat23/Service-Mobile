...
if($lineLib->checkBindAccount($user_id)){
	...
	$getPeriodKP = $conoracle->prepare("SELECT recv_period,kpslip_no AS RECEIPT_NO,receipt_date,receive_amt,keeping_status from kpmastreceive
										where member_no = :member_no and  (RECV_PERIOD = :recv_period or kpslip_no = :receipt_no )
	...
	$getDetailKP = $conoracle->prepare("SELECT 
		...
		$getDetailKP->execute([
			...
			':kpslip' => $arrayGroupPeriod[0]["SLIP_NO"]
		]);
	...
	if(sizeof($arrayGroupPeriod)>0){
		...
		//$arrPostData["replyToken"] = $list["DETAIL"]; 
	}else{
