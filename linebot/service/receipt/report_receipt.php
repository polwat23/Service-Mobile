...
function generatePdf($dataReport,$header,$lib){
	$sumBalance = 0;
	$html = '<style>
				@font-face {
...
function GenerateReport($member_no,$recv_period,$seq_no){
	...
	$arrayPDF = generatePdf($arrGroupDetail,$header,$lib);
	if($arrayPDF["RESULT"]){
		...
		$logStruc = [
			":error_menu" => $filename,
			":error_code" => "WS0044",
			":error_desc" => "Line Bot สร้าง PDF ไม่ได้ "."\n".json_encode($data),
			":error_device" => 'Line Bot'
		];
		$log->writeLog('errorusage',$logStruc);
		$message_error = "Line Bot สร้าง PDF ไม่ได้ ".$filename."\n"."DATA => ".json_encode($data);
