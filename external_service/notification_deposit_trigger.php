...
$fetchDataSTM = $conoracle->prepare("SELECT dsm.PRNCBAL,dsm.DEPTACCOUNT_NO,dit.DEPTITEMTYPE_DESC,dsm.DEPTITEM_AMT as AMOUNT,dm.MEMBER_NO,dsm.ENTRY_DATE,dsm.SEQ_NO
...
while($rowSTM = $fetchDataSTM->fetch(PDO::FETCH_ASSOC)){
	...
	foreach($arrToken["LIST_SEND"] as $dest){
		...
		$lib->convertdate($rowSTM["ENTRY_DATE"],'D m Y') : $lib->convertdate(date('Y-m-d H:i:s'),'D m Y');
