		...
		if($dataChkNotify == 1){
			...
			if($dataSendLib["RESULT"] == 1){
				...
				if($insertNotify->execute([
					...
					$message_error = "Line Bot insert เธฅเธ  lbnotify  เนเธกเนเนเธ”เน".''."\n".'data => '.$insertNotify;
				...
				if($insertNotNotify->execute([
					...
					$message_error = "Line Bot insert เธฅเธ  lbnotnotify  เนเธกเนเนเธ”เน".''."\n".'data => '.$dataMessage;
					$lib->sendLineNotify($message_error);
				}
			}
		}
...
//file_put_contents(__DIR__.'/lineNotfyRespone.txt', json_encode($exData,JSON_UNESCAPED_UNICODE ) . PHP_EOL, FILE_APPEND);
//print_r($exData);
//print_r($data);

//print_r($dataChkNotify);
