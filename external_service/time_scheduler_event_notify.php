...
$getNotifyWaitforSend = $conmysql->prepare("SELECT create_by,id_sendahead,send_topic,send_message,destination,id_smsquery,id_smstemplate,send_platform,send_image
							...
							if(isset($arrayTel[0]["TEL"]) && $arrayTel[0]["TEL"] != ""){
								...
								if($arraySendSMS["RESULT"]){
									...
									$bulkInsert[] = "('".$arrMessage["BODY"]."','".$arrayTel[0]["MEMBER_NO"]."',
											'sms','".$arrayTel[0]["TEL"]."',null,'".$arraySendSMS["MESSAGE"]."','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
								...
								$bulkInsert[] = "('".$arrMessage["BODY"]."','".$arrayTel[0]["MEMBER_NO"]."',
								'sms',null,null,'ไม่พบเบอร์โทรศัพท์','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
								if(sizeof($bulkInsert) == 1000){
									$func->logSMSWasNotSent($bulkInsert);
									unset($bulkInsert);
									$bulkInsert = array();
					...
					if(isset($dest["TEL"]) && $dest["TEL"] != ""){
						...
						if($arraySendSMS["RESULT"]){
							...
							$bulkInsert[] = "('".$message_body."','".$dest["MEMBER_NO"]."',
									'sms','".$dest["TEL"]."',null,'".$arraySendSMS["MESSAGE"]."','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
							if(sizeof($bulkInsert) == 1000){
								$func->logSMSWasNotSent($bulkInsert);
								unset($bulkInsert);
							}
			...
			foreach($arrToken["LIST_SEND"] as $dest){
				if(isset($dest["TOKEN"]) && $dest["TOKEN"] != ""){
					if($dest["RECEIVE_NOTIFY_NEWS"] == "1"){
						...
						'mobile_app',null,'".$dest["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
					}
					if(sizeof($bulkInsert) == 1000){
						$func->logSMSWasNotSent($bulkInsert);
						unset($bulkInsert);
					...
					$bulkInsert[] = "('".$rowNoti["send_message"]."','".$dest["MEMBER_NO"]."',
					'mobile_app',null,null,'หา Token ในการส่งไม่เจออาจจะเพราะไม่อนุญาตให้ส่งแจ้งเตือนเข้าเครื่อง','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
					if(sizeof($bulkInsert) == 1000){
						$func->logSMSWasNotSent($bulkInsert);
						unset($bulkInsert);
						$bulkInsert = array();
							...
							if(isset($arrToken["LIST_SEND"][0]["TOKEN"]) && $arrToken["LIST_SEND"][0]["TOKEN"] != ""){
								if($arrToken["LIST_SEND"][0]["RECEIVE_NOTIFY_NEWS"] == "1"){
									...
									if($lib->sendNotify($arrPayloadNotify,'person')){
										...
										$blukInsertNot[] = "('".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','mobile_app',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
									...
									$blukInsertNot[] = "('".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','mobile_app',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot);
										unset($blukInsertNot);
										$blukInsertNot = array();
								...
								$blukInsertNot[] = "('".$arrMessageMerge["BODY"]."','".$rowTarget[$rowQuery["target_field"]]."','mobile_app',null,null,'หา Token ในการส่งไม่เจออาจจะเพราะไม่อนุญาตให้ส่งแจ้งเตือนเข้าเครื่อง','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
								if(sizeof($blukInsertNot) == 1000){
									$func->logSMSWasNotSent($blukInsertNot);
									unset($blukInsertNot);
									$blukInsertNot = array();
							...
							if(isset($rowTarget[$rowQuery["target_field"]])){
								...
								if(sizeof($arrToken["MEMBER_NO"]) > 0){
									if(isset($arrToken["LIST_SEND"][0]["TOKEN"]) && $arrToken["LIST_SEND"][0]["TOKEN"] != ""){
										...
										$blukInsertNot[] = "('".$arrMessageMerge["BODY"]."','".$target."','mobile_app',null,null,'หา Token ในการส่งไม่เจออาจจะเพราะไม่อนุญาตให้ส่งแจ้งเตือนเข้าเครื่อง','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
										if(sizeof($blukInsertNot) == 1000){
											$func->logSMSWasNotSent($blukInsertNot);
											unset($blukInsertNot);
											$blukInsertNot = array();
									...
									$blukInsertNot[] = "('".$arrMessageMerge["BODY"]."','".$target."','mobile_app',null,null,'สมาชิกยังไม่ได้ใช้งานแอปพลิเคชั่น','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot);
										unset($blukInsertNot);
										$blukInsertNot = array();
				...
				foreach($arrToken["LIST_SEND"] as $dest){
					if(isset($dest["TOKEN"]) && $dest["TOKEN"] != ""){
						...
						if($lib->sendNotify($arrPayloadNotify,'person')){
							...
							$blukInsertNot[] = "('".$message."','".$dest["MEMBER_NO"]."','mobile_app',null,'".$dest["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$rowNoti["create_by"]."',".$rowNoti["id_smstemplate"].")";
							if(sizeof($blukInsertNot) == 1000){
								$func->logSMSWasNotSent($blukInsertNot);
								unset($blukInsertNot);
								$blukInsertNot = array();
