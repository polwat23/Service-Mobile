...
set DBUSER=root
set DBPASS=@CGD2020
set DBNAME=mobile_cgd
set PROJECT_PATH=C:\Mobile\service-cgd\
set BACKUP_PATH=%PROJECT_PATH%backup\

IF NOT EXIST %BACKUP_PATH% mkdir %BACKUP_PATH%

"C:\Program Files\MariaDB 10.4\bin\mysqldump.exe" -h%DBHOST% -P%DBPORT% -u %DBUSER% -p%DBPASS% %DBNAME% > %BACKUP_PATH%%DBNAME%.sql
