...
$fetchDataSTM = $conmssql->prepare("SELECT SHS.SEQ_NO,SHS.OPERATE_DATE,SHS.MEMBER_NO,(SHS.SHARE_AMOUNT * 10) AS AMOUNT,
									...
									WHERE SHS.OPERATE_DATE BETWEEN (GETDATE() - 50) and GETDATE() AND (SHS.sync_notify_flag IS NULL OR SHS.sync_notify_flag = '0') AND SHS.SHRITEMTYPE_CODE IN(".implode(',',$arrayStmItem).")");
