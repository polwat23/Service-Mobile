...
$templateMessage = $func->getTemplateSystem('PayAssistPension',1);
if(isset($templateMessage)){
	$queryTarget = $conoracle->prepare("SELECT slo.assistslip_no,slo.member_no,slo.payoutnet_amt,slo.slip_date
										FROM assslippayout slo LEFT JOIN assucfassisttype slu ON slo.assisttype_code = slu.assisttype_code
										WHERE slo.slip_date BETWEEN (SYSDATE - 2) and SYSDATE and slo.assisttype_code = '80' and 
										slo.sync_notify_flag = '0' and slo.member_no = '00017583'");
	$queryTarget->execute();
