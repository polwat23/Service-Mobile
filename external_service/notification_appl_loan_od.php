...
$fetchDataSTM = $conoracle->prepare("SELECT LOANREQUEST_DOCNO,MEMBER_NO,APPROVE_DATE,LOANTYPE_CODE
									FROM lnreqloan 
									where approve_date BETWEEN (SYSDATE - 3) and SYSDATE
									and LOANTYPE_CODE = '09'
	...
	foreach($arrToken["LIST_SEND"] as $dest){
		if($dest["RECEIVE_NOTIFY_TRANSACTION"] == '1'){
			...
			if($lib->sendNotify($arrPayloadNotify,"person")){
				...
				$updateSyncFlag = $conoracle->prepare("UPDATE lnreqloan SET sync_notify_flag = '1' WHERE LOANREQUEST_DOCNO = :loanrequest_docno");
