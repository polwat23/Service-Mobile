	...
	if($func->check_permission_core($payload,'mobileadmin','loanrequestformmember')){
		...
		if($checkAccBankBeenbind->rowCount() > 0){
			...
			if($insertIntoInfo->execute([
				...
				$insertbindaccount = $conmysql->prepare("UPDATE gcbindaccount SET deptaccount_no_bank=:deptaccount_no_bank WHERE member_no = :member_no");
				if($insertbindaccount->execute([
					':deptaccount_no_bank' => $dataComing["bank_account_no"],
					':member_no' => $member_no
				])){
					$conmysql->commit();
			...
			if($insertIntoInfo->execute([
				...
				$insertIntoInfo = $conmysql->prepare("INSERT INTO gcbindaccount(sigma_key,member_no,deptaccount_no_coop,citizen_id,mobile_no,bank_account_name,bank_account_name_en,bank_code,id_token) 
													  VALUES(:sigma_key,:member_no,:coop_account_no,:citizen_id,:mobile_no,:bank_account_name,:bank_account_name_en,'006',:id_token");
				if($insertIntoInfo->execute([
					':sigma_key' => $sigma_key,
					':member_no' => $member_no,
