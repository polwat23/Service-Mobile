...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','documentrequest')){
		...
		$getReqDocument = $conoracle->prepare("SELECT * FROM mem_t_candidate 
			WHERE BR_NO = '121' and ROWNUM <= 10");
		...
		while($rowReqDocument = $getReqDocument->fetch(PDO::FETCH_ASSOC)){
			...
			$formValue["ADDRESS_DISTRICT_CODE"]["VALUE"]["DISTRICT_DESC"] = $rowReqDocument[""];
			...
			$formValue["ADDRESS_PROVINE_CODE"]["VALUE"]["PROVINCE_DESC"] = $rowReqDocument[""];
