...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','transactionschedulelist')){
		...
		$getSchedule = $conmysql->prepare("SELECT ts.id_transchedule, ts.member_no, ts.transaction_type, ts.from_account, ts.destination,
										...
										tss.scheduler_date, ts.transaction_date, ts.amt_transfer, ts.bank_code, ts.id_userlogin, 
										ts.app_version, ts.scheduler_status , cs.bank_short_name, tss.scheduler_status as scheduler_statement_status,
                                        tss.scheduler_detail, tss.process_date
										FROM gctransactionschedulestatement tss
                                        LEFT JOIN gctransactionschedule ts ON ts.id_transchedule = tss.id_transchedule
                                        LEFT JOIN csbankdisplay cs ON ts.bank_code = cs.bank_code
										ORDER BY tss.process_date desc");
		...
		while($rowSchedule = $getSchedule->fetch(PDO::FETCH_ASSOC)){
			...
			if($rowSchedule["scheduler_statement_status"] == "1"){
				$arrRePay["SCHEDULER_STATEMENT_STATUS"] = "เสร็จแล้ว";
				$arrRePay["STATEMENT_STATUS_COLOR"] = '#00917c';
			}else if($rowSchedule["scheduler_statement_status"] == "-99"){
				$arrRePay["SCHEDULER_STATEMENT_STATUS"] = "ล้มเหลว";
				$arrRePay["STATEMENT_STATUS_COLOR"] = '#c64756';
			}
			$arrRePay["SCHEDULER_DETAIL"] = $rowSchedule["scheduler_detail"];
			$arrRePay["PROCESS_DATE"] = $rowSchedule["process_date"];
			
			$arrGrp[] = $arrRePay;
		}
			
		$arrayResult['STATEMENT_SCHEDULE'] = $arrGrp;
