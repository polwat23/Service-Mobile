...
if($lib->checkCompleteArgument(['unique_id','deptaccount_no'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','deptaccountjoint')){
		...
		$fetchDeptInfo = $conoracle->prepare("SELECT dp.ACC_TYPE as depttype_code,dt.ACC_DESC as depttype_desc,dp.account_no as deptaccount_no,dp.account_id as MEMBER_NO,
