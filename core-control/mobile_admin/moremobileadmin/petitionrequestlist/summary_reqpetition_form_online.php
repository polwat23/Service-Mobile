...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','petitionrequestlist')){
		...
		$getAllReqDocno = $conmysql->prepare("SELECT COUNT(reqdoc_no) AS COUNT_WAITING,
															(SELECT COUNT(reqdoc_no) AS COUNT_PROCESSING FROM gcpetitionformreq WHERE req_status = '7'".$filterQuery.") AS COUNT_PROCESSING,
															(SELECT COUNT(reqdoc_no) AS COUNT_CANCEL FROM gcpetitionformreq WHERE req_status = '9'".$filterQuery.") AS COUNT_CANCEL,
															(SELECT COUNT(reqdoc_no) AS COUNT_DISAPPROVAL FROM gcpetitionformreq WHERE req_status = '-9'".$filterQuery.") AS COUNT_DISAPPROVAL,
															(SELECT COUNT(reqdoc_no) AS COUNT_APPROVE FROM gcpetitionformreq WHERE req_status = '1'".$filterQuery.") AS COUNT_APPROVE
															FROM gcpetitionformreq WHERE req_status = '8'".$filterQuery);
