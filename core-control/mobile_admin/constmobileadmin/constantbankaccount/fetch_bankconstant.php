...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','constantdeptaccount',$conoracle)){
		$arrayGroup = array();
		$fetchConstant = $conoracle->prepare("SELECT id_bankconstant,transaction_name, TRIM(transaction_cycle) as transaction_cycle, max_numof_deposit, 
										max_numof_withdraw, min_deposit, max_deposit, min_withdraw, max_withdraw, each_bank FROM gcbankconstant
										WHERE is_use = '1'");
