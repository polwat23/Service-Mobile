...
if($lib->checkCompleteArgument(['unique_id','member_no','section','minimum_value'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','constantapprwithdrawal',$conoracle)){
		if(isset($dataComing["minimum_value"]) || $dataComing["minimum_value"] != ''){
			$updateConstants = $conoracle->prepare("INSERT INTO gcconstantapprwithdrawal(minimum_value, maximum_value, member_no, id_section_system) 
																	VALUES (:minimum_value,:maximum_value,:member_no,:id_section_system)");
			if($updateConstants->execute([
				':minimum_value' => $dataComing["minimum_value"]??null,
