...
if($lib->checkCompleteArgument(['unique_id','loandata'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','constanttypeloan')){
		...
		$fetchLoanTypeCheck = $conmysql->prepare("SELECT LOANTYPE_CODE,IS_CREDITLOAN,IS_LOANREQUEST,IS_ESTIMATE_CREDITLOAN,IS_QRPAYMENT FROM gcconstanttypeloan");
		...
		if($dataComing["loandata"] !== $arrayGroup){
			...
			foreach($resultUDiff as $value_diff){
				if(array_search($value_diff["LOANTYPE_CODE"],array_column($arrayLoanCheckGrp,'LOANTYPE_CODE')) === False){
					...
					$updateConst = $conmysql->prepare("UPDATE gcconstanttypeloan SET IS_CREDITLOAN = :IS_CREDITLOAN,IS_LOANREQUEST = :IS_LOANREQUEST, 
														IS_ESTIMATE_CREDITLOAN = :IS_ESTIMATE_CREDITLOAN,IS_QRPAYMENT = :IS_QRPAYMENT WHERE LOANTYPE_CODE = :LOANTYPE_CODE");
					$updateConst->execute([
					...
					$updateConstLog = 'LOANTYPE_CODE=> '.$value_diff["LOANTYPE_CODE"].' IS_CREDITLOAN ='.$value_diff["IS_CREDITLOAN"].' IS_LOANREQUEST='.$value_diff["IS_LOANREQUEST"].' IS_ESTIMATE_CREDITLOAN='.$value_diff["IS_ESTIMATE_CREDITLOAN"].' IS_QRPAYMENT='.$value_diff["IS_QRPAYMENT"];
				}
			}
			$insertConst = $conmysql->prepare("INSERT gcconstanttypeloan(LOANTYPE_CODE,IS_CREDITLOAN,IS_LOANREQUEST,IS_ESTIMATE_CREDITLOAN,IS_QRPAYMENT)
