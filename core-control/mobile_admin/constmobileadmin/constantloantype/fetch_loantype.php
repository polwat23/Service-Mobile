...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','constanttypeloan')){
		...
		$fetchLoanTypeCheck = $conmysql->prepare("SELECT IS_NOTIFY,LOANTYPE_CODE,IS_CREDITLOAN,IS_LOANREQUEST,IS_ESTIMATE_CREDITLOAN FROM gcconstanttypeloan");
		...
		while($rowLoantype = $fetchLoantype->fetch(PDO::FETCH_ASSOC)){
			...
			if(array_search($rowLoantype["LOANTYPE_CODE"],array_column($arrayLoanCheckGrp,'LOANTYPE_CODE')) === False){
				...
				$arrayLoantype["IS_NOTIFY"] = $arrayLoanCheckGrp[array_search($rowLoantype["LOANTYPE_CODE"],array_column($arrayLoanCheckGrp,'LOANTYPE_CODE'))]["IS_NOTIFY"];
