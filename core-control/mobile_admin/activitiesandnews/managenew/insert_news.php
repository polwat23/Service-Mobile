...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','managenews')){
		$stmt = $conmysql->prepare("SELECT MAX(id_gallery)  AS max_id, path_img_1 AS img  FROM gcgallery ");
		$stmt->execute();
		$invNum = $stmt -> fetch(PDO::FETCH_ASSOC);
		$last_id_gallary = $invNum['max_id'];
		$img1 = $invNum['img'];
		$insert_news	= $conmysql->prepare("INSERT INTO gcnews (news_title, news_detail, path_img_header,  link_news_more, id_gallery, create_by)
