...
if($lib->checkCompleteArgument(['unique_id','coop_branch_id','queuedate'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','loanrequestqueues')){
		$arrayGroup = array();
		$fetchBranch = $conmysql->prepare("SELECT qmt.queue_id,qmt.coop_branch_id,qmt.max_queue,qmt.queue_date,qmt.queue_starttime,qmt.queue_endtime,qmt.remain_queue,qmt.queue_status,queue_type FROM gcloanreqqueuemaster qmt
