...
if($lib->checkCompleteArgument(['unique_id','coop_branch_id','queue_type'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','loanrequestqueues')){
		$arrayGroup = array();
		$fetchBranch = $conmysql->prepare("SELECT queue_date,coop_branch_id,SUM(max_queue) as max_queue,SUM(remain_queue) as remain_queue FROM gcloanreqqueuemaster WHERE queue_status = '1' AND coop_branch_id = :coop_branch_id AND queue_type = :queue_type 
