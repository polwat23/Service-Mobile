...
if($lib->checkCompleteArgument(['unique_id','queue_date','coop_branch_id','queue_type'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','loanrequestqueues')){
		$arrayGroup = array();
		$fetchBranch = $conmysql->prepare("SELECT qmt.queue_id, qmt.coop_branch_id, qmt.max_queue, qmt.queue_date, qmt.queue_starttime, qmt.queue_endtime, qmt.queue_status, qmt.remain_queue, queue_type
														FROM gcloanreqqueuemaster qmt
														WHERE qmt.queue_date = :queue_date AND qmt.coop_branch_id = :coop_branch_id AND qmt.queue_type = :queue_type");
		$fetchBranch->execute([
		...
		while($rowBranch = $fetchBranch->fetch(PDO::FETCH_ASSOC)){
			...
			$arrGroupUserAcount["QUEUE_TYPE"] = $rowBranch["queue_type"];
			...
			$fetchMember= $conmysql->prepare("SELECT member_no,is_use,queuedt_id FROM gcloanreqqueuedetail WHERE queue_id = :queue_id");
			$fetchMember->execute([
