...
if($lib->checkCompleteArgument(['unique_id','member_no'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','manageuseraccount')){
		...
		$fetchAccount = $conoracle->prepare("SELECT member_no,memb_name as acc_name,memb_ename as acc_surname,addr_email as email FROM mbmembmaster  where member_no =  :member_no");
