...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','suspendingdebtqueues')){
		$arrayGroup = array();
		$fetchBranch = $conmysql->prepare("SELECT queue_date,coop_branch_id,SUM(max_queue) as max_queue,SUM(remain_queue) as remain_queue FROM gcsusdebtqueuemaster WHERE queue_status = '1' AND coop_branch_id = :coop_branch_id 
