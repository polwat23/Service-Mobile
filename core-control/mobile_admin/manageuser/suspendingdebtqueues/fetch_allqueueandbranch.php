...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','suspendingdebtqueues')){
		...
		$arrayGroup = array();
		$fetchQueue = $conmysql->prepare("SELECT queue_date,coop_branch_id, max_queue, remain_queue,queue_starttime,queue_endtime,queue_id
		FROM gcsusdebtqueuemaster WHERE queue_status = '1'
