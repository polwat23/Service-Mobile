...
if($lib->checkCompleteArgument(['unique_id','id_const_welfare','input_type','input_name','label_text'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','manageassistance')){
		...
		if($dataComing["input_length_spc_"] == ""){
			$input_length_spc_ = 0;
		}else{
			$input_length_spc_ = $dataComing["input_length_spc_"];
		}
		$updaeusername = $conmysql->prepare("INSERT INTO gcformatreqwelfare (id_const_welfare, input_type, input_length, input_name, label_text, placeholder, default_value, 
										input_format,is_required) 
										VALUES (:id_const_welfare,:input_type,:input_length,:input_name,:label_text,:placeholder,:default_value,:input_format,:is_required)");
		if($updaeusername->execute([
			':id_const_welfare' => $dataComing["id_const_welfare"],
			':input_type' => $dataComing["input_type"],
			':input_length' => $input_length_spc_,
