...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','manage')){
		...
		while($rowBindAccount = $fetchidUserallowAcctrantion->fetch(PDO::FETCH_ASSOC)){
			...
			$fetchBindAcount = $conmysql->prepare("SELECT id_bindaccount,member_no,deptaccount_no_coop,deptaccount_no_bank,mobile_no,bank_account_name,bank_account_name_en,consent_date,bind_date,unbind_date,bindaccount_status,csbankdisplay.bank_short_name AS 'bank_name'
													FROM gcbindaccount 
			...
			$bindaccount["BIND_DATE"] = $dataBindAcount["bind_date"];
			$bindaccount["CONSENT_DATE"] = $dataBindAcount["consent_date"];
