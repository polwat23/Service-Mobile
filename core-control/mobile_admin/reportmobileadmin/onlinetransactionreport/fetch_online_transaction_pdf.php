	...
	if($func->check_permission_core($payload,'mobileadmin','depttransaction')){
		...
		while($rowRecon = $fetchReconcile->fetch(PDO::FETCH_ASSOC)){
			...
			if($rowRecon["transfer_mode"] == '9'){
				if($rowRecon["transaction_type_code"] == 'WTX'){ //ถอนภายนอก
		...
		$arrayResult['SUM_AMT_EXPORT'] = number_format($sum_deposit_external+$sum_withdraw_external+$sum_deposit_inside+$summary_feeamt+$penalty_amt ?? '0',2);
