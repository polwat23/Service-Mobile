...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'mobileadmin','onlinetransactionreport')){
		...
		while($rowRecon = $fetchReconcile->fetch(PDO::FETCH_ASSOC)){
			...
			if($rowRecon["transfer_mode"] == '9'){
				...
				if($rowRecon["transaction_type_code"] == 'WIM'){ //ถอนภายใน
					...
					$rowLontype = $getLontype->fetch(PDO::FETCH_ASSOC);
					$getRePayLoan = $conmysql->prepare("SELECT amount, fee_amt, penalty_amt,amount_receive FROM gctransaction WHERE ref_no = :ref_no");
					$getRePayLoan->execute([
						':ref_no' => $rowRecon["ref_no"]
					]);
					$rowRePayLoan = $getRePayLoan->fetch(PDO::FETCH_ASSOC);
					if(isset($arrayLoanExternalList[$rowLontype["LOANTYPE_CODE"]])){
						$arrayLoanExternalList[$rowLontype["LOANTYPE_CODE"]]["PRN_AMT"] += ($rowRePayLoan["amount"] ?? 0);
						$arrayLoanExternalList[$rowLontype["LOANTYPE_CODE"]]["INT_AMT"] += ($rowRePayLoan["amount_receive"] ?? 0);
						$arrayLoanExternalList[$rowLontype["LOANTYPE_CODE"]]["COUNT"]++;
					}else{
						$arrayLoanExternal[] = $rowLontype["LOANTYPE_CODE"];
						$arrData = array();
						$arrData["LOANTYPE_DESC"] = $rowLontype["LOANTYPE_DESC"];
						$arrData["PRN_AMT"] = $rowRePayLoan["amount"] ?? 0;
						$arrData["INT_AMT"] = $rowRePayLoan["amount_receive"] ?? 0;
						$arrData["COUNT"] = 1;
					...
					if($rowWithdraw["DEPTTYPE_CODE"] =='01'){ 
						...
						$inside_withdraw10++;
					}else if($rowWithdraw["DEPTTYPE_CODE"] =='04'){
						$payment20 = ($payment_inside_01 ?? 0) + ($payment_inside_02 ?? 0) + ($payment_inside_03 ?? 0);
						$inside_withdraw20++;
					}else if($rowWithdraw["DEPTTYPE_CODE"] =='05'){
					...
					*/
					...
					if($rowLontype["LOANGROUP_CODE"] == '17'){ //ชำระหนี้ ฉฉ
						...
						if($rowDept["DEPTTYPE_CODE"] =='01'){ 
							...
							//$inside_dept10++;
							 $withdraw_inside_10 +=  $rowRecon["amount"];
						}else if($rowDept["DEPTTYPE_CODE"] =='04'){
							//$depositinside_20 += ($getloan_inside_01 ?? 0);
							$inside_dept20++;
						}else if($rowDept["DEPTTYPE_CODE"] =='05'){
		...
		//รวมโอนภายใน
		$sum_deposit_inside = ($deposit_inside_10 ?? 0) + ($deposit_inside_20 ?? 0) + ($deposit_inside_70 ?? 0) + ($payment_inside_01 ?? 0) + ($payment_inside_02 ?? 0) + ($payment_inside_03 ?? 0);
		...
		foreach($arrayLoanExternal as $value){
			$arrInsideList[] = "- ".$arrayLoanExternalList[$value]["LOANTYPE_DESC"]." : ".($arrayLoanExternalList[$value]["COUNT"] ?? 0)." รายการ ".number_format($arrayLoanExternalList[$value]["PRN_AMT"] ?? '0',2)."  บาท";
		}
		//ถอน
		...
		$arrayResult['rowRePayLoan'] = $rowRePayLoan;
