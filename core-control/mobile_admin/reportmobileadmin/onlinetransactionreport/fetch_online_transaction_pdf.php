	...
	if($func->check_permission_core($payload,'mobileadmin','onlinetransactionreport')){
		...
		while($rowRecon = $fetchReconcile->fetch(PDO::FETCH_ASSOC)){
			...
			if($rowRecon["transfer_mode"] == '9'){
				if($rowRecon["transaction_type_code"] == 'WTX'){//โอนฝากภายนอก 	
					...
					if($rowDepttype["DEPTTYPE_CODE"] =='02'){
						$deposit_external_10 += $rowRecon["amount"];
						$dept_count10++;
					}else if($rowDepttype["DEPTTYPE_CODE"] =='88'){
						$deposit_external_88 += $rowRecon["amount"];
						$dept_count88++;
					}else if($rowDepttype["DEPTTYPE_CODE"] =='04'){
						$deposit_external_04 += $rowRecon["amount"];
						$dept_count04++;
					}
				}else if($rowRecon["transaction_type_code"] == 'WTX'){ //ถอนภายนอก
					...
					if($rowDepttype["DEPTTYPE_CODE"] =='02'){
						...
						$withdraw_external_88 += $rowRecon["amount"];
						$withdraw88++;
					}else if($rowDepttype["DEPTTYPE_CODE"] =='04'){
						$withdraw_external_04 += $rowRecon["amount"];
						$withdraw04++;
					}					
				}else if($rowRecon["transaction_type_code"] == 'WFS'){ //โอนชำระหนี้
					$getLontype = $conoracle->prepare("SELECT lty.loangroup_code FROM lncontmaster ln LEFT JOIN lnloantype lty ON ln.loantype_code = lty.loantype_code  WHERE ln.loancontract_no = :destination");
					$getLontype->execute([
						':destination' => $rowRecon["destination"]
					]);
				...
				if($rowRecon["transfer_mode"] == '1'){
					if($rowRecon["transaction_type_code"] == 'WIM'){//โอนฝากภายใน	
						...
						if($rowDepttype["DEPTTYPE_CODE"] =='02'){
							...
							$deposit_inside_88 += $rowRecon["amount"];
							$inside_dept88++;					
						}else if($rowDepttype["DEPTTYPE_CODE"] =='04'){
							$deposit_inside_04 += $rowRecon["amount"];
							$inside_dept04++;					
						}
					}
					if($rowRecon["transaction_type_code"] == 'WIM'){ //ถอนภายใน
						...
						if($rowDepttype["DEPTTYPE_CODE"] =='02'){
							...
							$withdraw_inside_88 += $rowRecon["amount"];
							$inside_withdraw88++;
						}
						else if($rowDepttype["DEPTTYPE_CODE"] =='04'){
							$withdraw_inside_04 += $rowRecon["amount"];
							$inside_withdraw04++;
						}
						
					}
				}else if($rowRecon["transfer_mode"] == '2'){
					if($rowRecon["transaction_type_code"] == 'WFS'){ //โอนชำระหนี้ภายใน
						$getLontype = $conoracle->prepare("SELECT lty.loangroup_code FROM lncontmaster ln LEFT JOIN lnloantype lty ON ln.loantype_code = lty.loantype_code  WHERE ln.loancontract_no = :destination");
						$getLontype->execute([
							':destination' => $rowRecon["destination"]
						]);
						...
						if($rowWithdraw["DEPTTYPE_CODE"] =='01'){ 
							...
							$inside_withdraw10++;
							$inside_dept10 ++;
						}else if($rowWithdraw["DEPTTYPE_CODE"] =='88'){
					...
					if($rowRecon["transaction_type_code"] == 'WFS'){ //ถอนเงินฝากไปชื้อหุ้น
						$getDepttype = $conoracle->prepare("SELECT depttype_code FROM dpdeptmaster  WHERE deptaccount_no = :from_account");
						$getDepttype->execute([
							':from_account' => $rowRecon["from_account"]
						]);
						...
						$deposit_inside_share  += $rowRecon["amount"];
						$inside_deptshare++;			
					}
				}else if($rowRecon["transfer_mode"] == '4'){
					if($rowRecon["transaction_type_code"] == 'DAP'){ //จ่ายเงินกู้
						$getLontype = $conoracle->prepare("SELECT lty.loangroup_code FROM lncontmaster ln LEFT JOIN lnloantype lty ON ln.loantype_code = lty.loantype_code  WHERE ln.loancontract_no = :from_account");
						$getLontype->execute([
							':from_account' => $rowRecon["from_account"]
						]);
						...
						if($rowLontype["loangroup_code"] == '01'){ //ชำระหนี้ ฉฉ
							...
							if($rowDept["DEPTTYPE_CODE"] =='01'){ 
								...
								$inside_dept10++;
							}else if($rowDept["DEPTTYPE_CODE"] =='88'){
		...
		$withdraw_inside_88  = ($withdraw_inside_88 ?? 0) + ($payment20 ?? 0);	
		$withdraw_inside_04  = ($withdraw_inside_04 ?? 0) + ($payment20 ?? 0);	
		
		$deposit_inside_10 = ($deposit_inside_10 ?? 0) + ($depositinside_10 ?? 0); //จ่ายเงิยกู้
		$deposit_inside_88 = ($deposit_inside_88 ?? 0) + ($depositinside_20 ?? 0);
		$deposit_inside_04 = ($deposit_inside_04 ?? 0) + ($depositinside_04 ?? 0);
		//รวมโอนภายนอก
		$sum_deposit_external = ($deposit_external_10 ?? 0)  + ($deposit_external_88 ?? 0)  + ($deposit_external_04 ?? 0)+ ($payment_external_01 ?? 0) + ($payment_external_02 ?? 0) + ($payment_external_03 ?? 0);
		$sum_withdraw_external = ($withdraw_external_10 ?? 0) + ($withdraw_external_88 ?? 0)  +($withdraw_external_04 ?? 0) + ($getloan_external_01 ?? 0);

		//รวมโอนภายใน
		$sum_deposit_inside = ($deposit_inside_10 ?? 0) + ($deposit_inside_88 ?? 0) +($deposit_inside_04 ?? 0) + ($payment_inside_01 ?? 0) + ($payment_inside_02 ?? 0) + ($payment_inside_03 ?? 0) + ($deposit_inside_share ?? 0); 
		$sum_withdraw_inside = ($withdraw_inside_10 ?? 0) + ($withdraw_inside_88 ?? 0) + ($withdraw_inside_04 ?? 0) + ($getloan_inside_01 ?? 0) ;
		...
		$arrayResult['DEPOSIT_INSIDE_20'] = number_format($deposit_inside_88 ?? '0',2);
		$arrayResult['INSIDE_DEPT20'] = $inside_dept88 ?? 0;
		$arrayResult['DEPOSIT_INSIDE_04'] = number_format($deposit_inside_04?? '0',2);
		$arrayResult['INSIDE_DEPT04'] = $inside_dept04 ?? 0;
		$arrayResult['DEPOSIT_INSIDE_SHARE'] = number_format($deposit_inside_share?? '0',2);
		$arrayResult['INSIDE_DEPTSHARE'] = $inside_deptshare ?? 0;
		$arrayResult['WITHDRAW_INSIDE_10'] = number_format($withdraw_inside_10 ?? '0',2);
		$arrayResult['INSIDE_WITHDRAW10'] = $inside_withdraw10 ?? 0;
		$arrayResult['WITHDRAW_INSIDE_20'] = number_format($withdraw_inside_88 ?? '0',2);
		$arrayResult['INSIDE_WITHDRAW20'] = $inside_withdraw88 ?? 0;
		$arrayResult['WITHDRAW_INSIDE_04'] = number_format($withdraw_inside_04 ?? '0',2);
		$arrayResult['INSIDE_WITHDRAW04'] = $inside_withdraw04 ?? 0;
		...
		$arrayResult['DEPOSIT_EXTERNAL_20'] = number_format($deposit_external_88 ?? '0',2);
		$arrayResult['DEPT_COUNT20'] = $dept_count88 ?? 0;
		$arrayResult['DEPOSIT_EXTERNAL_04'] = number_format($deposit_external_04 ?? '0',2);
		$arrayResult['DEPT_COUNT04'] = $dept_count04 ?? 0;
		$arrayResult['WITHDRAW_EXTERNAL_10'] = number_format($withdraw_external_10 ?? '0',2);
		$arrayResult['WITHDRAW10'] = $withdraw10 ?? 0;
		$arrayResult['WITHDRAW_EXTERNAL_20'] = number_format($withdraw_external_88 ?? '0',2);
		$arrayResult['WITHDRAW20'] = $withdraw88 ?? 0;
		$arrayResult['WITHDRAW_EXTERNAL_04'] = number_format($withdraw_external_04 ?? '0',2);
		$arrayResult['WITHDRAW04'] = $withdraw04 ?? 0;
