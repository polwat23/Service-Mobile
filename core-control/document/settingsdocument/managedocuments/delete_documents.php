...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'settingsdocument','managedocuments')){
		...
		while(count($checkedFolders) > 0){
			...
			$fetchDocumentSystems = $conmssql->prepare("SELECT docgrp_no
		...
		if(count($deleteGrp) > 0){
			//delete subfolders
			$deleteDocuments = $conmssql->prepare("UPDATE docgroupcontrol SET is_use = '0' WHERE is_use = '1' AND (docgrp_ref in ('".implode("','",$deleteGrp)."') OR docgrp_no in ('".implode("','",$dataComing["checkedFolders"])."'))");
			...
			//delete subfiles
			$deleteFiles = $conmssql->prepare("UPDATE doclistmaster SET doc_status = '-9' WHERE doc_status = '1' AND docgrp_no in ('".implode("','",$deleteGrp)."')");
		...
		if(count($deleteCheckedFiles) > 0){
			$deleteFiles = $conmssql->prepare("UPDATE doclistmaster SET doc_status = '-9' WHERE doc_status = '1' AND doc_no in ('".implode("','",$deleteCheckedFiles)."')");
			if($deleteFiles->execute()){
