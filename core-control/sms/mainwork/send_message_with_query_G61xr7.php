...
if($lib->checkCompleteArgument(['unique_id','message_emoji_','type_send','channel_send'],$dataComing)){
	if($func->check_permission_core($payload,'sms','sendmessageall',$conmssql) 
		|| $func->check_permission_core($payload,'sms','sendmessageperson',$conmssql)){
		...
		if($dataComing["channel_send"] == "mobile_app"){
			...
			while($rowTarget = $getNormCont->fetch(PDO::FETCH_ASSOC)){
				...
				if(!in_array($rowTarget["member_no"].'_'.$arrMessageMerge["BODY"],$dataComing["destination_revoke"])){
					...
					if(sizeof($arrToken["MEMBER_NO"]) > 0){
							...
							if($arrToken["LIST_SEND"][0]["RECEIVE_NOTIFY_TRANSACTION"] == "1"){
								...
								if($lib->sendNotify($arrPayloadNotify,$dataComing["type_send"])){
									...
									if(sizeof($blukInsert) == 1000){
										...
										$func->insertHistory($arrPayloadHistory,'2','0',$conmssql);
										unset($blukInsert);
										$blukInsert = array();
									}
								}else{
									$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot,$conmssql);
										unset($blukInsertNot);
										$blukInsertNot = array();
									}
								}
							}else{
								$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
								if(sizeof($blukInsertNot) == 1000){
									$func->logSMSWasNotSent($blukInsertNot,$conmssql);
									unset($blukInsertNot);
									$blukInsertNot = array();
							...
							if(isset($arrToken["LIST_SEND_HW"][0]["TOKEN"]) && $arrToken["LIST_SEND_HW"][0]["TOKEN"] != ""){
								if($arrToken["LIST_SEND_HW"][0]["RECEIVE_NOTIFY_TRANSACTION"] == "1"){
									...
									if($lib->sendNotifyHW($arrPayloadNotify,$dataComing["type_send"])){
										...
										$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND_HW"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND_HW"][0]["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
										if(sizeof($blukInsertNot) == 1000){
											$func->logSMSWasNotSent($blukInsertNot,$conmssql);
											unset($blukInsertNot);
											$blukInsertNot = array();
									...
									$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$arrToken["LIST_SEND_HW"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND_HW"][0]["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot,$conmssql);
										unset($blukInsertNot);
										$blukInsertNot = array();
								...
								$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$target."','".$dataComing["channel_send"]."',null,null,'หา Token ในการส่งไม่เจออาจจะเพราะไม่อนุญาตให้ส่งแจ้งเตือนเข้าเครื่อง','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
								if(sizeof($blukInsertNot) == 1000){
									$func->logSMSWasNotSent($blukInsertNot,$conmssql);
									unset($blukInsertNot);
									$blukInsertNot = array();
						...
						$blukInsertNot[] = "('".$arrMessageMerge["SUBJECT"]."','".$arrMessageMerge["BODY"]."','".$target."','".$dataComing["channel_send"]."',null,null,'สมาชิกยังไม่ได้ใช้งานแอปพลิเคชั่น','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
						if(sizeof($blukInsertNot) == 1000){
							$func->logSMSWasNotSent($blukInsertNot,$conmssql);
							unset($blukInsertNot);
							$blukInsertNot = array();
			...
			$arrayMerge = array();
			$bulkInsert = array();
			$id_smsnotsent = $func->getMaxTable('id_smsnotsent' , 'smswasnotsent',$conmssql);
			$getNormCont = $conmssql->prepare("SELECT sld.ITEM_PAYAMT,sl.MEMBER_NO,sld.SLIPITEM_DESC,TO_CHAR(sl.entry_date,'YYYY-MM-DD HH24:MI') as ENTRY_DATE,NVL(sld.LOANCONTRACT_NO,sl.MEMBER_NO) as LOANCONTRACT_NO
					...
					if(isset($arrayTel[0]["TEL"]) && $arrayTel[0]["TEL"] != ""){
						...
						if($arraySendSMS["RESULT"]){
							...
							if(sizeof($bulkInsert) == 1000){
								$func->logSMSWasNotSent($bulkInsert,$conmssql);
								unset($bulkInsert);
								$bulkInsert = array();
							}
						}
			...
			if(sizeof($arrGRPAll) > 0){
				$arrayLogSMS = $func->logSMSWasSent($id_template,$arrGRPAll,$arrayMerge,$payload["username"],$conmssql,true);
