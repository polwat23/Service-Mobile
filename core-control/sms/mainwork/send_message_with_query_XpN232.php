	...
	if($func->check_permission_core($payload,'sms','sendmessageall',$conoracle) 
		...
		if($dataComing["channel_send"] == "mobile_app"){
			...
			foreach($dataComing["destination"] as $target){
				...
				$getCoopPhone = $conoracle->prepare("SELECT IVR_PASSWORD,MEMBER_NO FROM MBMEMBMASTER WHERE member_no = :member_no");
				$getCoopPhone->execute([':member_no' => $destination]);
				$rowTarget = $getCoopPhone->fetch(PDO::FETCH_ASSOC);
				if(isset($rowTarget["IVR_PASSWORD"])){
					...
					if(!in_array($target.'_'.str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"]),$dataComing["destination_revoke"])){
						...
						if(sizeof($arrToken["MEMBER_NO"]) > 0){
							if(isset($arrToken["LIST_SEND"][0]["TOKEN"]) && $arrToken["LIST_SEND"][0]["TOKEN"] != ""){
								if($arrToken["LIST_SEND"][0]["RECEIVE_NOTIFY_TRANSACTION"] == "1"){
									...
									if($lib->sendNotify($arrPayloadNotify,$dataComing["type_send"])){
										$blukInsert[] = "('".$id_history."','1','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".($pathImg ?? null)."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
										...
										$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
									...
									$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$arrToken["LIST_SEND"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND"][0]["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot,$conoracle);
										unset($blukInsertNot);
										$blukInsertNot = array();
								...
								if(isset($arrToken["LIST_SEND_HW"][0]["TOKEN"]) && $arrToken["LIST_SEND_HW"][0]["TOKEN"] != ""){
									if($arrToken["LIST_SEND_HW"][0]["RECEIVE_NOTIFY_TRANSACTION"] == "1"){
										...
										if($lib->sendNotifyHW($arrPayloadNotify,$dataComing["type_send"])){
											$blukInsert[] = "('".$id_history."','1','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".($pathImg ?? null)."','".$arrToken["LIST_SEND_HW"][0]["MEMBER_NO"]."','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
											if(sizeof($blukInsert) == 1000){
												$arrPayloadHistory["TYPE_SEND_HISTORY"] = "manymessage";
												$arrPayloadHistory["bulkInsert"] = $blukInsert;
												$func->insertHistory($arrPayloadHistory,'2','0',$conoracle);
											...
											$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$arrToken["LIST_SEND_HW"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND_HW"][0]["TOKEN"]."','ไม่สามารถส่งได้ให้ดู LOG','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
											if(sizeof($blukInsertNot) == 1000){
												$func->logSMSWasNotSent($blukInsertNot,$conoracle);
												unset($blukInsertNot);
												$blukInsertNot = array();
										...
										$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$arrToken["LIST_SEND_HW"][0]["MEMBER_NO"]."','".$dataComing["channel_send"]."',null,'".$arrToken["LIST_SEND_HW"][0]["TOKEN"]."','บัญชีปลายทางไม่ประสงค์เปิดรับการแจ้งเตือน','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
										if(sizeof($blukInsertNot) == 1000){
											$func->logSMSWasNotSent($blukInsertNot,$conoracle);
											unset($blukInsertNot);
											$blukInsertNot = array();
									...
									$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$target."','".$dataComing["channel_send"]."',null,null,'หา Token ในการส่งไม่เจออาจจะเพราะไม่อนุญาตให้ส่งแจ้งเตือนเข้าเครื่อง','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
									if(sizeof($blukInsertNot) == 1000){
										$func->logSMSWasNotSent($blukInsertNot,$conoracle);
										unset($blukInsertNot);
										$blukInsertNot = array();
							...
							$blukInsertNot[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessageMerge["BODY"])."','".$target."','".$dataComing["channel_send"]."',null,null,'สมาชิกยังไม่ได้ใช้งานแอปพลิเคชั่น','".$payload["username"]."'".(isset($id_template) ? ",".$id_template : ",null").")";
							if(sizeof($blukInsertNot) == 1000){
								$func->logSMSWasNotSent($blukInsertNot,$conoracle);
								unset($blukInsertNot);
								$blukInsertNot = array();
			...
			foreach($dataComing["destination"] as $target){
				...
				if(isset($rowTarget["IVR_PASSWORD"])){
					...
					if(!in_array($destination.'_'.str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessage["BODY"]),$dataComing["destination_revoke"])){
						...
						if(isset($arrayTel[0]["TEL"]) && $arrayTel[0]["TEL"] != ""){
							...
							if($arraySendSMS["RESULT"]){
								...
								$arrGRPAll[$arrayTel[0]["MEMBER_NO"]] = str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessage["BODY"]);
							}else{
								$bulkInsert[] = "('".$id_smsnotsent."','".$arrMessageMerge["SUBJECT"]."','".str_replace($rowTarget["IVR_PASSWORD"],' **** ',$arrMessage["BODY"])."','".$arrayTel[0]["MEMBER_NO"]."',
