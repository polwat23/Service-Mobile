...
if($lib->checkCompleteArgument(['unique_id','id_smstemplate'],$dataComing)){
	if($func->check_permission_core($payload,'sms','managetemplate')){
		...
		$fetchCheckTemplate = $conmssql->prepare("SELECT temp.id_submenu,temp.id_smstemplate,temp.is_use as temp_isuse,sub.menu_status FROM `smstopicmatchtemplate` temp
		...
		if($fetchCheckTemplate->fetch()){
			...
			$unuseTemplate = $conmssql->prepare("UPDATE smstemplate SET is_use = '-9' WHERE id_smstemplate = :id_smstemplate");
