		...
		if(isset($dataComing["id_smsquery"]) && $dataComing["id_smsquery"] != ""){
			if(isset($dataComing["query_template_spc_"]) && isset($dataComing["column_selected"])){
				if(empty($dataComing["condition_target"])){
					$updateSmsQuery = $conmssql->prepare("UPDATE smsquery SET sms_query = :sms_query,column_selected = :column_selected,
					...
					$updateSmsQuery = $conmssql->prepare("UPDATE smsquery SET sms_query = :sms_query,column_selected = :column_selected,is_stampflag = :is_stampflag,
				...
				$updateSmsQuery = $conmssql->prepare("UPDATE smsquery SET sms_query = null,column_selected = null,
			...
			$editTemplate = $conmssql->prepare("UPDATE smstemplate SET smstemplate_name = :smstemplate_name,smstemplate_body = :smstemplate_body
			...
			if(isset($dataComing["query_template_spc_"]) && $dataComing["query_template_spc_"] != "" && isset($dataComing["column_selected"])){
				if(empty($dataComing["condition_target"]) || $dataComing["condition_target"] == ""){
					...
					if($insertSmsQuery->execute([
						...
						$editTemplate = $conmssql->prepare("UPDATE smstemplate SET smstemplate_name = :smstemplate_name,smstemplate_body = :smstemplate_body,id_smsquery = :id_smsquery
														WHERE id_smstemplate = :id_smstemplate");
						if($editTemplate->execute([
							':smstemplate_name' => $dataComing["template_name"],
							':smstemplate_body' => $dataComing["template_body"],
