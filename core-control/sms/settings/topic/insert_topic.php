...
if($lib->checkCompleteArgument(['unique_id','id_template','topic_name'],$dataComing)){
	if($func->check_permission_core($payload,'sms','managetopic') && is_numeric($dataComing["id_template"])){
		...
		$insertSmsMenu = $conmssql->prepare("INSERT INTO coresubmenu(menu_name,page_name,menu_order,create_by,id_menuparent,id_coremenu)
		...
		if($insertSmsMenu->execute([
			...
			$id_submenu = $conmssql->lastInsertId();
			$insertTopicMatch = $conmssql->prepare("INSERT INTO smstopicmatchtemplate(id_submenu,id_smstemplate) 
