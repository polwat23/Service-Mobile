...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'sms','reportnotifysuccess')){
		...
		$fetchReport = $conmssql->prepare("SELECT id_history, his_type, his_title, his_detail, his_path_image, his_read_status, his_del_status, member_no, receive_date, read_date, send_by, id_smstemplate, is_sendahead
											...
											".(isset($dataComing["start_date"]) && $dataComing["start_date"] != '' ? "and convert(varchar,receive_date,23) >= :start_date" : null)."
											".(isset($dataComing["end_date"]) && $dataComing["end_date"] != '' ? "and convert(varchar,receive_date,23) <= :end_date" : null)." ORDER BY receive_date DESC");
