...
if($lib->checkCompleteArgument(['unique_id','username','status_permission'],$dataComing)){
	if($func->check_permission_core($payload,'admincontrol','permissionmenu')){
		...
		$insertSubMenuPermit = $conmssql->prepare("DELETE FROM corepermissionmenu WHERE username = :username");
		if($insertSubMenuPermit->execute([
			':username' => $dataComing["username"]
		])){
			$fetchCoreMenu = $conmssql->prepare("SELECT id_coremenu FROM coremenu WHERE coremenu_status = '1'");
			...
			$insertPermitCoreMenu = $conmssql->prepare("INSERT INTO corepermissionmenu(id_coremenu,username,is_use)
																			VALUES".implode(',',$bulkInsert));
			if($insertPermitCoreMenu->execute()){
				foreach($arrayGroupCoreMenu as $coreMenu_id){
					$checkSubmenu = $conmssql->prepare("SELECT id_permission_menu 
					...
					$fetchSubMenu_id = $conmssql->prepare("SELECT id_submenu FROM coresubmenu 
				...
				$insert_permiss_subcore = $conmssql->prepare("INSERT INTO corepermissionsubmenu (id_submenu,id_permission_menu,is_use)
