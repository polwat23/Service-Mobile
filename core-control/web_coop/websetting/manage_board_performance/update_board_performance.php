...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	...
	if(empty($dataComing["img_head"]) && $dataComing["img_head"] == null){
		$arrOldIMG=$dataComing[old_img];
		$old_img = $arrOldIMG[0];
		$del_IMG="../../../../".$old_img["path"];;
		$delFileIMG=unlink($del_IMG);	

		$arrayResult['RESULT'] = FALSE;
		$arrayResult['delFileIMG'] = $delFileIMG;
		$arrayResult['delFileIMG'] = $old_img["path"];
	...
	foreach($dataComing["file"] as $file_pdf){
		if($file_pdf["status"]=="old"){
			...
			if(isset($dataComing["file"]) && $dataComing["file"] != null){
				...
				if($createImage){
					$fileUrl = $config["URL_SERVICE"]."resource/gallery_web_coop/board_performance/".$createImage["normal_path"];
					$filePath = "resource/gallery_web_coop/board_performance/".$createImage["normal_path"];
					$fileName = $file_pdf["file_name"];
				}else{
					$arrayResult['RESPONSE_MESSAGE'] = "นามสกุลไฟล์ไม่ถูกต้อง";
					$arrayResult['RESULT'] = FALSE;
	...
	$update_board_performance = $conmysql->prepare("UPDATE
											...
											");
	if($update_board_performance->execute([
		...
		$arrayResult['RESPONSE'] = "ไม่สามารถอัพเดทข้อมูลได้ กรุณาติดต่อผู้พัฒนา  ";
		$arrayResult['RESULT'] = FALSE;
		echo json_encode($arrayResult);
		exit();
	}		
