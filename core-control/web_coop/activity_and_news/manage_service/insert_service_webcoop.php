...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	...
	if(isset($dataComing["img_head"]) && $dataComing["img_head"] != null){
		...
		if($createImage == 'oversize'){
			...
			if($createImage){
				$pathImgHeadNews = "resource/gallery_web_coop/service/".$createImage["normal_path"];
				$urlImgHeadNews = $config["URL_SERVICE"]."resource/gallery_web_coop/service/".$createImage["normal_path"];
			}else{
	...
	$detail_html = '<!DOCTYPE HTML>
	  ...
	  $insert_gallery = $conmysql->prepare("INSERT INTO webcoopgallary(
		...
		if($insert_gallery->execute([
			':gallery_name' =>  $dataComing["gallery_name"],
			...
			$fetchIdGallery = $conmysql->prepare("SELECT
													...
													");
			$fetchIdGallery->execute([':gallery_name' => $dataComing["gallery_name"]]);
			$id_Gallery = $fetchIdGallery->fetch(PDO::FETCH_ASSOC);	
			$insert_service_webcoop = $conmysql->prepare("INSERT INTO webcoopservice(
															title,
															detail,
															groupservice_id,
															id_gallery, 
													...
													");
			if($insert_service_webcoop->execute([
				...
				if(isset($dataComing["file"]) && $dataComing["file"] != null){
					...
					foreach($file_news as $file64){
						...
						if($createImage){
							$path_file["url"] = $config["URL_SERVICE"]."resource/gallery_web_coop/service/".$createImage["normal_path"];
							$path_file["path"] = "resource/gallery_web_coop/service/".$createImage["normal_path"];
