...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'line','lineresquickmsg')){
		...
		$fetchMsg = $conmysql->prepare("SELECT id_quickmsg,text,update_date FROM lbquickmessage WHERE is_use = '1' ORDER BY update_date DESC");
		$fetchMsg->execute();
			
		while($rowMsg = $fetchMsg->fetch(PDO::FETCH_ASSOC)){
			$fetchActions = $conmysql->prepare("SELECT  qmm.quickmessagemap_id,ac.id_action,ac.text,ac.type,ac.url,ac.area_x,ac.area_y,ac.width,ac.height,ac.label,ac.data,ac.data,ac.mode,ac.initial,ac.max,ac.min FROM  lbquickmessagemap qmm
			...
			while($rowAction = $fetchActions->fetch(PDO::FETCH_ASSOC)){
				...
				$arrAction["STATUS"] = 'old';
				$type = $rowAction["type"];
				$actions[]= $arrAction;
			}
			$arrMsg = array();
			$arrMsg["ID_QUICKMSG"] = $rowMsg["id_quickmsg"];
			$arrMsg["ID_QUICKMESSAGEMAP"] = $quickmessagemap_id;
