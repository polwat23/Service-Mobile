...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'line','constans')){
		$arrayGroup = array();
		$fetchConstant = $conmysql->prepare("SELECT id_constant ,constant_name,constant_value,constant_desc,initial_value,is_dropdown FROM lbconstant WHERE is_use = '1'");
