...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	...
	if($payload["section_system"] == "root" || $payload["section_system"] == "root_test"){
		$fetchMenu = $conmssql->prepare("SELECT id_coremenu,coremenu_name,coremenu_iconpath,coremenu_status,
										coremenu_desc,coremenu_colorbanner,root_path,coremenu_colortext
										FROM coremenu
										WHERE coremenu_parent = '0' ORDER BY coremenu_order ASC");
		$fetchMenu->execute();
	}else{
		$fetchMenu = $conmssql->prepare("SELECT cm.id_coremenu,cm.coremenu_name,cm.coremenu_iconpath,cm.coremenu_status,
	...
	while($rowMenu = $fetchMenu->fetch(PDO::FETCH_ASSOC)){
		if($payload["section_system"] == "root" || $payload["section_system"] == "root_test"){
			...
			$checkPermission = $conmssql->prepare("SELECT cpm.id_permission_submenu FROM corepermissionsubmenu cpm LEFT JOIN coresubmenu csm
