...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'log','loginlog')){
		$arrayGroup = array();
		$fetchBindAccountLog = $conmysql->prepare("SELECT  bind.id_bindaccount,bind.sigma_key,bind.member_no,bind.bank_account_name,bind.deptaccount_no_coop,bind.deptaccount_no_bank,
													bind.mobile_no,bind.consent_date,bind.bind_date,bind.bindaccount_status,token.device_name,token.ip_address
												FROM gcbindaccount bind
												INNER JOIN gctoken token
												ON token.id_token=bind.id_token");
