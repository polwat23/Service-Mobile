...
if($lib->checkCompleteArgument(['unique_id'],$dataComing)){
	if($func->check_permission_core($payload,'log','logwithdrawerror')){
		$arrayGroup = array();
		$fetchTranfertError = $conmysql->prepare("SELECT tran.id_transferinsidecoop,tran.member_no,tran.transaction_date,tran.deptaccount_no,tran.amt_transfer,
												tran.type_request,tran.transfer_flag,tran.destination,tran.response_code,tran.response_message,login.device_name,
												login.channel
