...
if($lib->checkCompleteArgument(['menu_component','amt_transfer','contract_no','sigma_key'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferDepPayLoan')){
		...
		if($rowBankDisplay["fee_deposit"] > 0){
			...
			$dataComing["amt_transfer"],$rowBankDisplay["fee_deposit"],$dateOperC,$config,$deptslip_no,$lib,$getlastseqFeeAcc["MAX_SEQ_NO"],$dataAccFee,true,$payinslip_no,$deptslip_noFee);
		...
		if($payslip["RESULT"]){
			...
			0,$int_returnSrc,$interestPeriod,'1',$penaltyWtd["DEPTSLIP_NO"]);
