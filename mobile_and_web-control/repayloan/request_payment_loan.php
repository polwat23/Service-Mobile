	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferDepPayLoan')){
		...
		if($checkSeqAmtSrc["CAN_WITHDRAW"]){
			...
			if($wtdResult["RESULT"]){	
				...
				$insertRepayloan = $conmysql->prepare("INSERT INTO gcrepayloan(loancontract_no,member_no,amount,id_userlogin)
														VALUES(:contract_no,:member_no,:amount,:id_userlogin)");
				if($insertRepayloan->execute([
