...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransactionDeposit')){
		...
		if($fetchBindAccount->rowCount() > 0){
			...
			$arrTypeAllow = array();
			$fetchAccountBeenAllow = $conmysql->prepare("SELECT gct.dept_type_code 
												FROM gcconstantaccountdept gct
												WHERE gct.allow_deposit_outside = '1'");
			$fetchAccountBeenAllow->execute();
			while($rowAccCoop = $fetchAccountBeenAllow->fetch(PDO::FETCH_ASSOC)){
				$arrTypeAllow[] = "'".$rowAccCoop["dept_type_code"]."'";
			}
			$getDataAcc = $conoracle->prepare("SELECT TRIM(dpm.deptaccount_name) as DEPTACCOUNT_NAME,DPT.DEPTTYPE_DESC,DPM.DEPTTYPE_CODE,
												...
												WHERE dpm.member_no = :member_no and dpm.deptclose_status = 0 and dpm.depttype_code IN(".implode($arrTypeAllow,',').")");
