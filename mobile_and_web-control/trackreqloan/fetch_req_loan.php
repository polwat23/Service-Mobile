		...
		if(isset($dataComing["req_status"]) && $dataComing["req_status"] != ""){
			...
			while($rowReqLoan = $fetchReqLoan->fetch(PDO::FETCH_ASSOC)){
				...
				if ($forceNewSecurity == true) {
					...
					if ($rowReqLoan["house_regis_img"]) {
						$path_explode = explode('/resource/', $rowReqLoan["house_regis_img"]);
						$resource_path = (sizeof($path_explode) > 1) ? "/resource/" . $path_explode[1] : "";
						if ($resource_path) {
							$arrayReq['HOUSE_REGIS_IMG'] = $config["URL_SERVICE"]."/resource/get_resource?id=".hash("sha256", $resource_path);
							$arrayReq["HOUSE_REGIS_IMG_TOKEN"] = $lib->generate_token_access_resource($resource_path, $jwt_token, $config["SECRET_KEY_JWT"]);
						}
					}
