	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'Election')){
		...
		if($dataComing["value_election"] == '2'){
			...
			require_once('../../include/exit_footer.php');
				...
				$getInitData = $conoracle->prepare("SELECT MP.PRENAME_SHORT,MB.MEMB_NAME,MB.MEMB_SURNAME,MB.CARD_PERSON,MB.MEM_TELMOBILE
																	FROM mbmembmaster mb LEFT JOIN mbucfprename mp ON mb.prename_code = mp.prename_code
																	WHERE mb.member_no = :member_no");
				$getInitData->execute([':member_no' => $member_no]);
				$rowData = $getInitData->fetch(PDO::FETCH_ASSOC);
				if(isset($rowData["MEM_TELMOBILE"]) && $rowData["MEM_TELMOBILE"] != ""){
					...
					$arrayResult['TEL'] = $rowData["MEM_TELMOBILE"];
