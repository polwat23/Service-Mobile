	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SlipInfo')){
		...
		if($lib->checkCompleteArgument(['seq_no'],$dataComing)){
			$getPaymentDetail = $conoracle->prepare("SELECT 
																		...
																		LEFT JOIN dpdepttype dp ON kpd.shrlontype_code = dp.depttype_code and mb.membcat_code = dp.membcat_code
		...
		while($rowDetail = $getPaymentDetail->fetch(PDO::FETCH_ASSOC)){
			...
			if($rowDetail["TYPE_GROUP"] == 'SHR'){
				...
				$arrDetail["PAY_ACCOUNT"] = $lib->formataccount(str_replace("เลขที่บัญชีเงินฝาก ","",$rowDetail["PAY_ACCOUNT"]),$func->getConstant('dep_format'));
...
function GenerateReport($dataReport,$header,$lib){
	...
	$html .= '<p style="margin-top: -30px;font-size: 22px;font-weight: bold">สหกรณ์ออมทรัพย์กรมทางหลวง จำกัด</p>
			<p style="margin-top: -27px;font-size: 18px;">อาคาร 26  2/486 ถนนศรีอยุธยา </p>
			<p style="margin-top: -25px;font-size: 18px;">แขวงทุ่งพญาไท เขตราชเทวี กรุงเทพมหานคร 10400</p>
			<p style="margin-top: -25px;font-size: 18px;">โทร : 02-345-6668 ต่อ 27014</p>
			<p style="margin-top: -27px;font-size: 19px;font-weight: bold">www.dohcoop.com</p>
			</div>
			</div>
