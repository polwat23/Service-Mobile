	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SlipInfo')){
		...
		$getPeriodKP = $conoracle->prepare("
			SELECT
				...
				SL.DEPTSLIP_AMT, 
				SL.COOP_ID,
				(select sum(PRNCSLIP_AMT) from wcdeptslipdet where deptslip_no = SL.DEPTSLIP_NO and  coop_id =   SL.COOP_ID) as total
		...
		while($rowPeriod = $getPeriodKP->fetch(PDO::FETCH_ASSOC)){
			...
			$arrKpmonth["RECEIVE_AMT"] = number_format($rowPeriod["TOTAL"],2);
