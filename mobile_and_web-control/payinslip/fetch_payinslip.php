	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'PayInSlip')){
		...
		$getCpctSlip = $conoracle->prepare("
			...
			where
				...
				and (wcdeptmaster.die_status is null or wcdeptmaster.die_status <> 1)
				order by wcrecievemonth.recv_period desc
			");
		$getCpctSlip->execute([
				':member_no' => $member_no
		]);
		while($rowCpct = $getCpctSlip->fetch(PDO::FETCH_ASSOC)){
			$arrCpctSlip = array();
			$arrCpctSlip["SLIP_NAME"] = "หนังสือแจ้งเรียกเก็บเงิน สสธท.";
