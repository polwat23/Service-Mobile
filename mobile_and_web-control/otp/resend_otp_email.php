...
if($lib->checkCompleteArgument(['member_no','email','ref_old_otp'],$dataComing)){
	...
	$arrayComing["EMAIL"] = $dataComing["email"];
	...
	if($insertOTP->execute([
		...
		':destination' => $dataComing["email"],
		':expire_date' => $expire_date,
		':otp_text' => $arrMessage["BODY"]
	])){
		$arrayDest["member_no"] = $member_no;
		$arrayDest["email"] = $arrayTel[0]["EMAIL"];
		$arrayDest["message"] = $arrMessage["BODY"];
		$arrMailStatus = $lib->sendMail($dataComing["email"],$arrMessage["SUBJECT"],$arrMessage["BODY"],$mailFunction);
