...
if($lib->checkCompleteArgument(['menu_component','loantype_code','request_amt','period_payment','period','loanpermit_amt'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanRequestForm')){
		...
		$principal_payment = 0;
		$principal_balance09 = 0;
		$principal_balance51 = 0;
		...
		if($getDocSystemPrefix->rowCount() > 0){
			...
			if(isset($reqloan_doc) && $reqloan_doc != ""){
				...
				$checkGroupControl = $conmysql->prepare("SELECT docgrp_no FROM docgroupcontrol WHERE docgrp_ref = :docgrp_ref 
														and is_use = '1' and docgrp_no = (YEAR(NOW()) + 543)");
				...
				$fetchData = $conoracle->prepare("SELECT mp.prename_desc,mb.memb_name,mb.memb_surname,mb.birth_date,
												mb.card_person,mb.salary_amount, mb.member_date,mst.position_desc,
												mg.membgroup_desc,
												mt.membtype_desc,
												mb.membgroup_code,
												...
												(sh.sharestk_amt * 10) AS SHAREBEGIN_AMT,sh.sharestk_amt,(sh.periodshare_amt * 10) as periodshare_amt, mb.email as email,mb.MEM_TELMOBILE as MEM_TELMOBILE, mariage_status
												...
												LEFT JOIN mbucfposition mst ON mb.position_code  = mst.position_code
				...
				if(isset($rowData["MEMBGROUP_CODE"]) && $rowData["MEMBGROUP_CODE"] != ""){
					$fetchGroupAddr = $conoracle->prepare("select 
													mb.mbgroup_phone as ADDR_PHONE,
													MBT.TAMBOL_DESC AS TAMBOL_DESC,
													MBD.DISTRICT_DESC AS DISTRICT_DESC,
													MBP.PROVINCE_DESC AS PROVINCE_DESC 
													from MBUCFMEMBGROUP mb
													LEFT JOIN MBUCFTAMBOL MBT ON mb.TAMBOL_CODE = MBT.TAMBOL_CODE
													LEFT JOIN MBUCFDISTRICT MBD ON mb.DISTRICT_CODE = MBD.DISTRICT_CODE
													LEFT JOIN MBUCFPROVINCE MBP ON mb.PROVINCE_CODE = MBP.PROVINCE_CODE
				...
				//if(true){
					...
					$fetchMaxRecv = $conoracle->prepare("select max(recv_period) as RECV_PERIOD  from kptempreceive where MEMBER_NO = :member_no");
					$fetchMaxRecv->execute([
						':member_no' => $member_no
					]);
					$rowMaxRecv = $fetchMaxRecv->fetch(PDO::FETCH_ASSOC);
					
					$fetchKptemp = $conoracle->prepare("select kpd.PRINCIPAL_PAYMENT  from kptempreceive kp LEFT JOIN kptempreceivedet kpd ON kp.kpslip_no = kpd.kpslip_no AND kp.recv_period = kpd.recv_period
														AND kp.member_no = kpd.member_no 
														where kp.member_no = :member_no
														and kpd.shrlontype_code in('09','10','11','12','13','14','15','16','17','18')
														and kp.recv_period = :recv_period ");
					$fetchKptemp->execute([
						':member_no' => $member_no,
						':recv_period' => $rowMaxRecv["RECV_PERIOD"]
					]);
					while($rowKptemp = $fetchKptemp->fetch(PDO::FETCH_ASSOC)){
						$principal_payment += $rowKptemp["PRINCIPAL_PAYMENT"];
					}

					
					$fetchEmerLoan = $conoracle->prepare("SELECT LM.PRINCIPAL_BALANCE,LT.LOANGROUP_CODE
													FROM LNCONTMASTER LM 
													JOIN LNLOANTYPE LT ON LT.LOANTYPE_CODE = LM.LOANTYPE_CODE 
													WHERE LM.MEMBER_NO =  :member_no
													AND LM.CONTRACT_STATUS = 1 AND LM.PRINCIPAL_BALANCE > 0");
					$fetchEmerLoan->execute([
						':member_no' => $member_no
					]);
					while($rowEmerLoan = $fetchEmerLoan->fetch(PDO::FETCH_ASSOC)){
						if($rowEmerLoan["LOANGROUP_CODE"] == '01'){
							$principal_balance_09 += $rowEmerLoan["PRINCIPAL_BALANCE"];
						}else {
							$principal_balance_51 += $rowEmerLoan["PRINCIPAL_BALANCE"];
						}
					}
					
			
					$fetchPrincipal = $conoracle->prepare("select  principal_balance , last_periodpay ,period_payamt    from lncontmaster where  member_no = :member_no and loantype_code = '09' 
														   and contract_status > 0 and contract_status <> 8 and principal_balance > 0 and (principal_balance - nkeep_principal > 0 OR nkeep_principal IS NULL)");
					$fetchPrincipal->execute([
						':member_no' => $member_no
					]);
					$rowPrincipal = $fetchPrincipal->fetch(PDO::FETCH_ASSOC);
					$arrData["deduct_debt09"] = $principal_balance_09  - $principal_payment;	
					$arrData["deduct_debt51"] = $principal_balance_51 - $principal_payment;				
					$arrData["principal_balance"] = $rowPrincipal["PRINCIPAL_BALANCE"];
					$arrData["last_periodpay"] = $rowPrincipal["LAST_PERIODPAY"];
					$arrData["period_payamt"] = $rowPrincipal["PERIOD_PAYAMT"];
					...
					$arrData["interest_rate"] =  $rowIntRate["INTEREST_RATE"];
				...
				$log->writeLog('errorusage',$logStruc);
				$message_error = "เลขเอกสารเป็นค่าว่าง ไม่สามารถสร้างเลขเอกสารเป็นค่าว่างได้".$rowDocPrefix["prefix_docno"];
				$lib->sendLineNotify($message_error);
