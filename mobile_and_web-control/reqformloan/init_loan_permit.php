		...
		if(isset($dataComing["request_amt"]) && $dataComing["request_amt"] != "" && isset($dataComing["period"]) && $dataComing["period"] != ""){
			...
			if($max_period == 0){
				...
				if($dataComing["option_paytype"] == "0"){
					...
					$period_payment = round($dataComing["request_amt"] * (($rowIntRate["INTEREST_RATE"] /100) / 12) / (1 - (exp(($dataComing["period"] * (-1)) * 
					log((1 + (($rowIntRate["INTEREST_RATE"] /100) / 12)))))));
				}
			}
			...
			if(isset($rowMaxPeriod["MAX_PERIOD"])){
				...
				$arrayResult["MAX_PERIOD"] = $max_period == 0 ? (string)$rowMaxPeriod["MAX_PERIOD"] : (string)$max_period;
				$arrayResult["PERIOD_PAYMENT"] = $period_payment;
