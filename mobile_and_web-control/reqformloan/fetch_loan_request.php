...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanRequestForm')){
		...
		$checkisAllowReq->execute([':member_no' => $payload["member_no"]]);
		
		$checkMemberType = $conmysql->prepare("SELECT member_no FROM gcmemberaccount WHERE (user_type = '9' OR user_type = '1') and member_no = :member_no");
		$checkMemberType->execute([':member_no' => $payload["member_no"]]);
		$rowMemberType = $checkMemberType->fetch(PDO::FETCH_ASSOC);
		...
		//if($checkisReq->rowCount() > 0 && ($typeMember != '9' && $typeMember != '8')){
			...
			while($rowIntRate = $fetchLoanIntRate->fetch(PDO::FETCH_ASSOC)){
					...
					if($rowIntRate["LOANTYPE_CODE"] == '23'){
						...
						if($rowIntRate["LOANTYPE_CODE"] == '41'){
							...
							$fetchContract = $conoracle->prepare("SELECT LM.STARTCONT_DATE,LM.LOANCONTRACT_NO,LM.PRINCIPAL_BALANCE, LM.LAST_PERIODPAY
															FROM LNCONTMASTER LM 
															WHERE LM.MEMBER_NO = :member_no AND LM.LOANTYPE_CODE IN('10')
															AND LM.CONTRACT_STATUS > 0 AND LM.CONTRACT_STATUS <> 8");
							...
							if($member_date_count < 24){
								$arrayDetailLoan["FLAG_NAME"] = "ต้องเป้นสมาชิกอย่างน้อย 24 เดือนขึ้นไป";
								$arrayDetailLoan["IS_REQ"] = FALSE;
							}else{
								$arrayDetailLoan["IS_REQ"] = TRUE;
							}
						...
						}else if($rowIntRate["LOANTYPE_CODE"] == '44'){
							// สามัญ สห
							...
							if(isset($rowContractGroup["LOANCONTRACT_NO"])){
								$arrayDetailLoan["FLAG_NAME"] = "ไม่สามารถกู้ได้เนื่องจากมีเงินกู้สามัญประเภทอื่นอยู่แล้ว";
								$arrayDetailLoan["IS_REQ"] = FALSE;
							}else if($member_date_count < 1){
								$arrayDetailLoan["FLAG_NAME"] = "ต้องเป้นสมาชิกอย่างน้อย 1 เดือนขึ้นไป";
								$arrayDetailLoan["IS_REQ"] = FALSE;
				...
				if($rowIntRate["LOANTYPE_CODE"] != '23'){
					if(isset($rowMemberType["member_no"])){
						if($rowIntRate["LOANTYPE_CODE"] == '41'){
							// สามัญ ก
							if($typeMember != '9'){
