...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DepositStatement')){
		...
		$fetchLastStmAcc = $conoracle->prepare("SELECT * from (SELECT TRIM(dps.deptaccount_no) as deptaccount_no,dt.depttype_desc,dpm.deptaccount_name,dpm.prncbal as BALANCE,
		...
		if(sizeof($arrSlipTrans) > 0){
			$fetchStmSeqDept = $conoracle->prepare("SELECT dpstm_no FROM dpdeptslip WHERE (deptslip_no IN('".implode("','",$arrSlipTrans)."') OR refer_slipno IN('".implode("','",$arrSlipTrans)."')) and TRIM(deptaccount_no) = :deptacc_no");
		...
		if(sizeof($arrSlipStm) > 0){
			$getStatement = $conoracle->prepare("SELECT * FROM (SELECT dit.DEPTITEMTYPE_DESC AS TYPE_TRAN,dit.SIGN_FLAG,dsm.seq_no,
												...
												WHERE TRIM(dsm.deptaccount_no) = :account_no and dsm.seq_no NOT IN('".implode("','",$arrSlipStm)."') and dsm.OPERATE_DATE 
			...
			$getStatement = $conoracle->prepare("SELECT * FROM (SELECT dit.DEPTITEMTYPE_DESC AS TYPE_TRAN,dit.SIGN_FLAG,dsm.seq_no,
												...
												FROM dpdeptstatement dsm LEFT JOIN DPUCFDEPTITEMTYPE dit
												ON dsm.DEPTITEMTYPE_CODE = dit.DEPTITEMTYPE_CODE 
												WHERE TRIM(dsm.deptaccount_no) = :account_no and dsm.OPERATE_DATE 
												BETWEEN to_date(:datebefore,'YYYY-MM-DD') and to_date(:datenow,'YYYY-MM-DD') ".$old_seq_no." 
												ORDER BY dsm.SEQ_NO DESC) WHERE rownum <= ".$rownum." ");
		}
