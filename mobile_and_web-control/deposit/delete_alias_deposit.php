...
if($lib->checkCompleteArgument(['menu_component','type_alias','account_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DepositInfo')){
		...
		if($dataComing["type_alias"] == 'alias_img'){
			$DeleteAliasDept = $conoracle->prepare("UPDATE gcdeptalias SET path_alias_img = null WHERE deptaccount_no = :deptaccount_no");
		}else if($dataComing["type_alias"] == 'alias_name'){
			$DeleteAliasDept = $conoracle->prepare("UPDATE gcdeptalias SET alias_name = null WHERE deptaccount_no = :deptaccount_no");
		}else{
			$DeleteAliasDept = $conoracle->prepare("UPDATE gcdeptalias SET path_alias_img = null,alias_name = null WHERE deptaccount_no = :deptaccount_no");
		}
