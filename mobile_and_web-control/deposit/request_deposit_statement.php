...
if($lib->checkCompleteArgument(['menu_component','account_no','request_date'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DepositStatement')){
		...
		$getCardPerson = $conoracle->prepare("SELECT card_person FROM mbmembmaster WHERE member_no = :member_no and member_status = '1' and branch_id = :branch_id");
		...
		foreach($dataComing["request_date"] as $date_between){
			$fetchDataSTM = $conoracle->prepare("SELECT dpt.DEPTITEMTYPE_DESC AS TYPE_TRAN,dpt.SIGN_FLAG,dps.REF_DOCNO as DEPTSLIP_NO,
