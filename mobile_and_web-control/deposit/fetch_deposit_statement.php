...
if($lib->checkCompleteArgument(['menu_component','account_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DepositStatement')){
		...
		$getAccount = $conoracle->prepare("SELECT prncbal as BALANCE FROM dpdeptmaster
											WHERE deptclose_status <> 1 and deptaccount_no = :account_no and branch_id = :branch_id ");
		$getAccount->execute([
		...
		if(sizeof($arrSlipTrans) > 0){
			$fetchStmSeqDept = $conoracle->prepare("SELECT dpstm_no FROM dpdeptslip WHERE (deptslip_no IN('".implode("','",$arrSlipTrans)."') 
													OR refer_slipno IN('".implode("','",$arrSlipTrans)."')) and deptaccount_no = :deptacc_no and branch_id = :branch_id");
