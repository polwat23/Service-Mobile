			...
			if(date('YmdHis',strtotime($rowCheckBill["expire_date"])) > date('YmdHis')){
				...
				while($rowDetail = $getDetailTran->fetch(PDO::FETCH_ASSOC)){
					if($rowDetail["trans_code_qr"] == '01'){ //ฝากเงิน
						...
						$depositMoney = $cal_dep->DepositMoneyInside($conmssql,$deptaccount_no,$vccAccID,'DIM',
						...
						if($paykeepingdet["RESULT"]){
							...
							$repayloan = $cal_loan->repayLoan($conmssql,$rowDetail["ref_account"],$rowDetail["qrtransferdt_amt"],0,$config,$payinslipdoc_no,$dateOper,
