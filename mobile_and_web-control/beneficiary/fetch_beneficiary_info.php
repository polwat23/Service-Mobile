...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'BeneficiaryInfo')){
		...
		$arrGroupBNF = array();
		$getBeneficiary = $conmssql->prepare("SELECT mg.GAIN_NAME,mg.GAIN_SURNAME,mg.GAIN_ADDRESS,mc.GAIN_CONCERN
		...
		while($rowBenefit = $getBeneficiary->fetch(PDO::FETCH_ASSOC)){
			...
			if(isset($rowBenefit["GAIN_ADDRESS"])){
				$arrBenefit["ADDRESS"] = preg_replace("/ {2,}/", " ", $rowBenefit["GAIN_ADDRESS"]);
			}
