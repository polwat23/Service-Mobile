...
if($lib->checkCompleteArgument(['menu_component','amt_transfer','deptaccount_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferDepBuyShare')){
		...
		$destvcid = $cal_dep->getVcMapID($constFromAcc["DEPTTYPE_CODE"],'SHR');
		...
		if($checkSeqAmtSrc["CAN_WITHDRAW"]){
			...
			if($wtdResult["RESULT"]){
				...
				if($payslip["RESULT"]){
					$getSharemasterinfo = $conoracle->prepare("SELECT (sharestk_amt * 10) as SHARE_AMT
					...
					$payslipdet = $cal_loan->paySlipDet($conoracle,$dataComing["amt_transfer"],$config,$dateOperC,$log,$payload,
					$from_account_no,$payinslip_no,'SHR','01','ซื้อหุ้นพิเศษ','1',null,$rowMastershare["SHARE_AMT"]);
					if($payslipdet["RESULT"]){
						$buyShare = $cal_share->buyShare($conoracle,$member_no,$dataComing["amt_transfer"],$dataComing["penalty_amt"],$config,$payinslipdoc_no,$dateOperC,
						...
						if($buyShare["RESULT"]){
							...
							$insertTransactionLog = $conmysql->prepare("INSERT INTO gctransaction(ref_no,transaction_type_code,from_account,destination_type,
																		...
																		VALUES(:ref_no,:slip_type,:from_account,'3',:destination,'3',:amount,:penalty_amt,
