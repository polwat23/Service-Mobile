	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferDepBuyShare')){
		...
		if($fetchAccAllowTrans->rowCount() > 0){
			...
			while($rowDataAccAllow = $getDataBalAcc->fetch(PDO::FETCH_ASSOC)){
				$checkSeqAmt = $cal_dep->getSequestAmt($rowDataAccAllow["DEPTACCOUNT_NO"]);
				if($checkSeqAmt["CAN_WITHDRAW"]){
					...
					$arrAccAllow["BALANCE"] = $cal_dep->getWithdrawable($rowDataAccAllow["DEPTACCOUNT_NO"]) - ($checkSeqAmt["SEQUEST_AMOUNT"] ?? 0);
			...
			if(sizeof($arrGroupAccAllow) > 0){
				$getConstantShare = $conoracle->prepare("SELECT UNITSHARE_VALUE FROM SHSHARETYPE");
