...
if($lib->checkCompleteArgument(['menu_component','bank_account_no','deptaccount_no','amt_transfer','sigma_key'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransactionWithdrawDeposit')){
		$fetchDataDeposit = $conmysql->prepare("SELECT csb.itemtype_wtd,csb.link_inquirywithd_coopdirect,gba.bank_code,csb.fee_withdraw
		...
		$getTypeAcc = $conoracle->prepare("SELECT DEPTTYPE_CODE,PRNCBAL FROM dpdeptmaster WHERE deptaccount_no = :acc_no");
		...
		$getContDeptType = $conoracle->prepare("SELECT MINWITD_AMT,NVL(s_maxwitd_inmonth,0) as MAXWITHD_INMONTH,NVL(withcount_flag,0) as IS_CHECK_PENALTY,MINPRNCBAL
		...
		$sumAllTransfer = $rowType["PRNCBAL"] - ($penalty_amt + $fee_amt + $amt_transfer);
		if($sumAllTransfer < $rowContDeptType["MINPRNCBAL"]){
