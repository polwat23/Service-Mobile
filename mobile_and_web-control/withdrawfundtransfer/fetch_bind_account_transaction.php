		...
		if($fetchBindAccount->rowCount() > 0){
			...
			if($fetchAccountBeenAllow->rowCount() > 0){
				...
				while($rowAccCoop = $fetchAccountBeenAllow->fetch(PDO::FETCH_ASSOC)){
					$getDataAcc = $conoracle->prepare("SELECT dpm.account_name as DEPTACCOUNT_NAME,dpt.ACC_DESC as DEPTTYPE_DESC,
					...
					if(isset($rowDataAcc["DEPTTYPE_DESC"])){
						...
						$arrAccCoop["BALANCE"] = $cal_dep->getWithdrawable($rowAccCoop["deptaccount_no"]);
