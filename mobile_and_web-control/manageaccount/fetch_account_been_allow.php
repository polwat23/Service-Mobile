...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'ManagementAccount')){
		...
		$fetchAccountBeenAllow = $conmysql->prepare("SELECT deptaccount_no,is_use,limit_transaction_amt FROM gcuserallowacctransaction WHERE member_no = :member_no and is_use <> '-9'");
