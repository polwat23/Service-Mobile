...
if($lib->checkCompleteArgument(['menu_component','slip_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SlipExtraPayment')){
		...
		$getPaymentDetail = $conmssql->prepare("SELECT SLD.SLIPITEM_DESC AS TYPE_DESC,SLD.STM_ITEMTYPE AS STM_ITEMTYPE,SLD.ITEM_BALANCE,SLD.PERIOD,
		...
		$getDetailSLHeader = $conmssql->prepare("SELECT 
												SLIPTYPE_CODE,SLIP_DATE,SLIP_STATUS,INTACCUM_AMT
												FROM SLSLIPPAYIN 
												WHERE payinslip_no = :payinslip_no");
...
function GenerateReport($dataReport,$header,$lib){
	...
	$html = '<style>
			...
			* {
			...
			<div style="display: flex;text-align: center;position: relative;margin-bottom: 20px;">
				<img src="../../resource/logo/logo.jpg" style="position: absolute; left: 230px;top: -16px" alt="" width="80" height="80" />
				<div>
				<div style="font-size: 22px; font-weight: bold;">สหกรณ์ออมทรัพย์พนักงานสยามคูโบต้า จำกัด</div>';
				if($header["slip_status"] == '1'){
					$html .= '<div style="margin-top: -5px;font-size: 22px;font-weight: bold;">ใบเสร็จรับเงิน</div>';
				}else{
					$html .= '<div style="margin-top: -5px;font-size: 22px;font-weight: bold;color: red;">ยกเลิกใบเสร็จรับเงิน</div>';
				}
				$html .= '
				</div>
			</div>
			...
			<td style="width: 50px;font-size: 18px;">เลขที่สมาชิก :</td>
			<td style="width: 101px;">'.$header["member_no"].'</td>
			</tr>
			<tr>
			<td style="width: 50px;font-size: 18px;">หน่วยงาน :</td>
			<td colspan="3" style="width: 350px;">'.$header["member_group"].'</td>
			</tr>
			</tbody>
			</table>
			</div>';
		$html .= '<div style="border: 0.5px solid black;width: 100%; height: 325px;">
			...
			<div style="width: 350px;text-align: center;font-size: 18px;font-weight: bold;border-right : 0.5px solid black;padding-top: 1px;">รายการ/สัญญา</div>
			...
			<div style="width: 120px;text-align: center;font-size: 18px;font-weight: bold;border-right : 0.5px solid black;margin-left: 700px;padding-top: 1px;">จำนวนเงิน</div>
			<div style="width: 150px;text-align: center;font-size: 18px;font-weight: bold;margin-left: 815px;padding-top: 1px;">คงเหลือ</div>
			</div>';
			// Detail
	...
	$html .= '<div style="display:flex;width: 100%;height: 40px" class="sub-table">
			...
			<div style="width: 110px;border-right: 0.5px solid white;height: 30px;margin-left: 465px;padding-top: 0px;">&nbsp;</div>
			...
			<div style="width:500px;font-size: 18px;">&nbsp;<br>&nbsp;
			</div>';
			$html .= '
			<div style="width:200px;margin-left: 700px;display:flex;">
			<img src="../../resource/utility_icon/signature/ts.jpg" width="80" height="30" style="margin-top:20px;"/>
			</div>
			</div>
			<div style="font-size: 18px;margin-left: 700px;margin-top:-50px;">ผู้จัดการ/เหรัญญิก</div>
			';
