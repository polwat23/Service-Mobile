...
if($lib->checkCompleteArgument(['menu_component','slip_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SlipExtraPayment')){
		...
		$getPaymentDetail = $conmssql->prepare("SELECT SLD.SLIPITEM_DESC AS TYPE_DESC,SLD.STM_ITEMTYPE AS STM_ITEMTYPE,SLD.ITEM_BALANCE,SLD.PERIOD,
												...
												WHERE LTRIM(RTRIM(SLD.PAYINSLIP_NO)) = :slip_no");
...
function GenerateReport($dataReport,$header,$lib){
	...
	if($header["sliptype_code"] == 'CLC'){
		...
		$html .= '<div style="position:absolute;top: 10px;right: 40px;font-size:22px;color:red">ใบเสร็จชำระหนี้ก่อนกำหนด</div>';
	}
	...
	$html .= '<div style="display:flex;width: 100%;height: 40px" class="sub-table">
			...
			<div style="font-size: 18px;margin-left: 730px;margin-top:0px;">เหรัญญิก</div>
			';
