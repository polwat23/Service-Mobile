...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SlipExtraPayment')){
		...
		$arrGrp = array();
		$limit_show_slip_extra = $func->getConstant("limit_show_slip_extra") ?? 0;
		$minYear = date("Y") - $limit_show_slip_extra + 1;
		$getListCountSlip = $conmssql->prepare("SELECT year(SLIP_DATE) AS EACH_YEAR,COUNT(PAYINSLIP_NO) AS C_SLIP 
										FROM SLSLIPPAYIN WHERE MEMBER_NO = :member_no AND year(SLIP_DATE) >= :limit_show_slip_extra
										GROUP BY year(SLIP_DATE) 
										ORDER BY year(SLIP_DATE) DESC");
		$getListCountSlip->execute([
			':member_no' => $member_no,
			':limit_show_slip_extra' => $minYear 
		]);
		while($rowListCountSlip = $getListCountSlip->fetch(PDO::FETCH_ASSOC)){
			...
			$getMonthSlip = $conmssql->prepare("SELECT month(SLIP_DATE) AS EACH_MONTH
												FROM SLSLIPPAYIN WHERE MEMBER_NO = :member_no and year(SLIP_DATE) = :year
												GROUP BY month(SLIP_DATE) ORDER BY month(SLIP_DATE) ASC");
			...
			while($rowMonth = $getMonthSlip->fetch(PDO::FETCH_ASSOC)){
				...
				$getListSlip = $conmssql->prepare("SELECT PAYINSLIP_NO FROM SLSLIPPAYIN 
													WHERE member_no = :member_no and year(SLIP_DATE) = :year 
													and month(SLIP_DATE) = :month");
				$getListSlip->execute([
					':member_no' => $member_no,
					':year' => $rowListCountSlip["EACH_YEAR"],
