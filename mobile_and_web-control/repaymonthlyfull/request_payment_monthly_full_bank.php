...
if($lib->checkCompleteArgument(['menu_component','amt_transfer','slip_no','sigma_key'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'PayMonthlyFull')){
				...
				if($cancelSlipLoan->execute([
					...
					if($rowKPDetail["KEEPITEMTYPE_GRP"] == 'LON'){
						...
						if($paykeepingdet["RESULT"]){
							...
							$vccAccID,null,$log,$lib,$payload,$from_account_no,$payinslip_no,$member_no,$ref_noLN,$dataComing["app_version"],0,true);
							if($repayloan["RESULT"]){
							}else{
								$conoracle->rollback();
								$conmysql->rollback();
		...
		if($arrResponse->RESULT){
			...
			$insertProcessMonthly = $conmysql->prepare("INSERT INTO gcprocesspaymentmonthly(ref_no,kpslip_no,amt_transfer,id_token,json_deposit,bank_code,account_no,member_no,payload)
														VALUES(:ref_no,:kpslip_no,:amt_transfer,:id_token,:json_deposit,:bank_code,:account_no,:member_no,:payload)");
			$insertProcessMonthly->execute([
				':ref_no' => $ref_no,
				':kpslip_no' => $dataComing['slip_no'],
				':amt_transfer' => $dataComing['amt_transfer'],
