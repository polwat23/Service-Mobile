...
if($lib->checkCompleteArgument(['user_type','member_no'],$payload) && $lib->checkCompleteArgument(['menu_component','k_mobile_no','citizen_id','kb_account_no'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],$conmysql,'BindAccountConsent')){
		...
		$arrPayload["external_reference"] = $configLB["external_reference"];
		$arrPayload["auth_parameter"] = $configLB["pass_phrase"];
		$arrPayloadverify = array();
		$arrPayloadverify['member_no'] = $member_no;
		$arrPayloadverify['account_no'] = preg_replace('/-/','',$dataComing["kb_account_no"]);
		$arrPayloadverify["coop_key"] = $config["COOP_KEY"];
		$arrPayloadverify["reference"] = $arrPayload["external_reference"];
