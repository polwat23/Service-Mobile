...
if($lib->checkCompleteArgument(['menu_component','trans_code','trans_amount','destination'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'GenerateQR')){
		...
		if($dataComing["trans_code"] == '01'){
			...
			$fetchLoanRepay = $conmssql->prepare("SELECT PRINCIPAL_BALANCE,INTEREST_RETURN,RKEEP_PRINCIPAL,LAST_PERIODPAY,
													LOANTYPE_CODE
													FROM lncontmaster
			...
			$interest = $cal_loan->calculateInterestArr($dataComing["destination"],$dataComing["trans_amount"]);
