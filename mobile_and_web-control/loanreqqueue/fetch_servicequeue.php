...
if($lib->checkCompleteArgument(['menu_component','coop_branch_id','queue_date','queue_type'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanRequestQueue')){
		$arrayGroup = array();
		$currentTime = date("H.i");
		$fetchBranch = $conmysql->prepare("SELECT qmt.queue_id, qmt.coop_branch_id, qmt.max_queue, qmt.queue_date, qmt.queue_starttime, qmt.queue_endtime, qmt.queue_status,qmt.remain_queue,qmt.queue_type
