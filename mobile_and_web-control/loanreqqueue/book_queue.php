...
if($lib->checkCompleteArgument(['menu_component','queue_id'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanRequestQueue')){
		...
		if($fetchBranch->rowCount() > 0){
			...
			$insertConst = $conmysql->prepare("INSERT INTO gcloanreqqueuedetail (queue_id, member_no) VALUES (:queue_id, :member_no)");
			if($insertConst->execute([
				':queue_id' => $dataComing["queue_id"],
				':member_no' => $member_no
			])){
				// update remain
				$updatemaster = $conmysql->prepare("UPDATE gcloanreqqueuemaster SET remain_queue = remain_queue-1 WHERE queue_id = :queue_id");
				if($updatemaster->execute([
					...
					$logStruc = [
						":error_menu" => $filename,
						":error_code" => "WS1038",
						":error_desc" => "ไม่สามารถ updatemaster ลง gcloanreqqueuemaster ได้ "."\n".$updatemaster->queryString."\n".json_encode([
							':queue_id' => $dataComing["queue_id"]
						]),
						":error_device" => $dataComing["channel"].' - '.$dataComing["unique_id"].' on V.'.$dataComing["app_version"]
					];
					$log->writeLog('errorusage',$logStruc);
					$message_error = "ไม่สามารถ updatemaster ลง gcloanreqqueuemaster ได้ "."\n".$updatemaster->queryString."\n".json_encode([
						':queue_id' => $dataComing["queue_id"]
					]);
				...
				$logStruc = [
					":error_menu" => $filename,
					":error_code" => "WS1038",
					":error_desc" => "ไม่สามารถ Insert ลง gcloanreqqueuedetail ได้ "."\n".$insertConst->queryString."\n".json_encode([
						':queue_id' => $dataComing["queue_id"],
				...
				$log->writeLog('errorusage',$logStruc);
				$message_error = "ไม่สามารถ Insert ลง gcloanreqqueuedetail ได้ "."\n".$insertConst->queryString."\n".json_encode([
					':queue_id' => $dataComing["queue_id"],
					':member_no' => $dataComing["member_no"]
				]);
				$lib->sendLineNotify($message_error);
