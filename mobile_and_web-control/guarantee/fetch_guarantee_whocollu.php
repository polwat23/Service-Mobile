...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'GuaranteeInfo')){
		...
		if(isset($dataComing["contract_no"])){
			...
			$arrGrpAllLoan = array();
			$getCollDetail = $conoracle->prepare("SELECT DISTINCT lnc.LOANCOLLTYPE_CODE,llc.LOANCOLLTYPE_DESC,lnc.REF_COLLNO,lnc.COLLACTIVE_AMT,
																lnc.DESCRIPTION
			...
			while($rowColl = $getCollDetail->fetch(PDO::FETCH_ASSOC)){
				...
				if($rowColl["LOANCOLLTYPE_CODE"] == '01'){
					...
					$arrGroupAllMember["DEPT_AMT"] = number_format($rowColl["COLLACTIVE_AMT"],2);
