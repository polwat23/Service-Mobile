	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'GuaranteeInfo')){
		...
		while($rowUcollwho = $getUcollwho->fetch(PDO::FETCH_ASSOC)){
			...
			if ($forceNewSecurity == true) {
				$arrayColl["AVATAR_PATH"] = null;
				$arrayColl["AVATAR_PATH_WEBP"] = null;
				...
				if (isset($arrayAvarTar["AVATAR_PATH_WEBP"])) {
					$arrayColl["AVATAR_PATH_WEBP"] = $config["URL_SERVICE"]."/resource/get_resource?id=".hash("sha256", $arrayAvarTar["AVATAR_PATH_WEBP"]);
					$arrayColl["AVATAR_PATH_WEBP_TOKEN"] = $lib->generate_token_access_resource($arrayAvarTar["AVATAR_PATH_WEBP"], $jwt_token, $config["SECRET_KEY_JWT"]);
				}
			}else{
