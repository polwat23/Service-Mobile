...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DeptTicket')){
		...
		while($rowAccount = $getAccount->fetch(PDO::FETCH_ASSOC)){
			...
			$arrAccount['SUM_BALANCE'] += $rowAccount["BALANCE"];
	...
	* {
	...
	$html.='<div>
		  ...
		  <div style="  line-height:25px;">
			...
			<div class="text-center text-bold">รายงานตั๋วสัญญาใช้เงินคงเหลือ</div>
			...
			<table>
			  <thead>
				<tr>
				  ...
				  <th style="width:100px;">เลขที่ P/N</th> 
	  ...
	  $html.=     '<tr>
					 ...
					 <td style="text-align:right;">'.($arrData["BALANCE"]??null).'</td>  
	...
	$pathfile = $pathfile.'/ticket'.$member_no.'.pdf';
	$pathfile_show = '/resource/pdf/payment/ticket'.$member_no.'.pdf?v='.time();
	$arrayPDF = array();
