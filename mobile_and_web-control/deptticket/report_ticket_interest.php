...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DeptTicketIntRate')){
		...
		$arrDetail = array();
		$arrAccount = array();
		$getAccount = $conoracle->prepare("SELECT DEPTACCOUNT_NO,TRIM(MEMBER_NO) as MEMBER_NO,DEPTACCOUNT_NAME,BALANCE_AMT,SUBSTR(DEPTPASSBOOK_NO,5) as DEPTPASSBOOK_NO ,PRNC_DATE,INTEREST_RATE,DEPTTYPE_CODE,DEPTTYPE_DESC,
									...
									WHERE ( DPDEPTMASTER.DEPTACCOUNT_NO = DPDEPTPRNCFIXED.DEPTACCOUNT_NO )   
										...
										DPDEPTMASTER.DEPTPASSBOOK_NO,DPDEPTPRNCFIXED.PRNC_DATE,DPDEPTPRNCFIXED.INTEREST_RATE,DPDEPTPRNCFIXED.PRNC_AMT,DPDEPTMASTER.DEPTTYPE_CODE,
										DPDEPTTYPE.DEPTTYPE_DESC ORDER BY  DPDEPTPRNCFIXED.PRNC_DATE ASC)");
		$getAccount->execute([
				':member_no' => $member_no,
				':datenow' => $date_now
			]);
		while($rowAccount = $getAccount->fetch(PDO::FETCH_ASSOC)){
			...
			$arrAccount["SUM_INTEREST"] += $lib->round_up($arrAccount["COUNT_DATE"] * $rowAccount["BALANCE_AMT"] * $rowAccount["INTEREST_RATE"] /36500);
	...
	* {
	...
	$html.='<div>
		  ...
		  <div style="  line-height:25px;">
			...
			<div class="text-center text-bold">รายงานตั๋วสัญญาใช้เงินดอกเบี้ยค้างจ่าย</div>
	  ...
	  $html.=     '<tr>
					 ...
					 <td style="text-align:right; padding-right:15px;">'.($arrData["INT_AMT"]??null).'</td> 
				  </tr>';
	...
	$html.='    
				...
				<div style="border:1px solid; "></div>
					...
					<td style="border-top:1px solid; "></td>
					<td style="border-top:1px solid; "</td>
					<td style="border-top:1px solid; text-align:right; padding-right:15px; " >'.$sum_int_amt.'<div  style="border-bottom:1px solid; "></div></td> 
					<td style="border-top:1px solid; "></td>
					<td style="border-top:1px solid; "></td>
				</tr>
	...
	$pathfile = $pathfile.'/ticketint'.$member_no.'.pdf';
	$pathfile_show = '/resource/pdf/payment/ticketint'.$member_no.'.pdf?v='.time();
	$arrayPDF = array();
