		...
		$member_no = $configAS[$payload["member_no"]] ?? $payload["member_no"];
				...
				if($updatemaster->execute([
					':queue_id' => $dataComing["queue_id"]
						...
						// update remain
						$updatedetail = $conmysql->prepare("UPDATE gcqueuedetail SET is_use = '0', update_by = :member_no WHERE queuedt_id = :queuedt_id");
						if($updatedetail->execute([
							...
							$logStruc = [
								":error_menu" => $filename,
								":error_code" => "WS1038",
								":error_desc" => "ไม่สามารถ update ลง gcqueuedetail ได้ "."\n".$updatedetail->queryString."\n".json_encode([
									':queuedt_id' => $dataComing["queuedt_id"],
							':member_no' => $member_no
								]),
							...
							$message_error = "ไม่สามารถ update ลง gcqueuemaster ได้ "."\n".$updatemaster->queryString."\n".json_encode([
								':queue_id' => $dataComing["queue_id"],
							':member_no' => $member_no
							]);
