	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'ShareInfo')){
		...
		if($rowMastershare){
			...
			$getShareStatement = $conoracle->prepare("SELECT stm.operate_date,(stm.share_amount * 10) as PERIOD_SHARE_AMOUNT,
														...
														WHERE stm.member_no = :member_no and stm.shritemtype_code NOT IN ('B/F','DIV') and to_char(stm.OPERATE_DATE,'YYYY-MM-DD')
														BETWEEN :datebefore and :datenow ORDER BY stm.seq_no DESC");
