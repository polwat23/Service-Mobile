	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DocBalanceConfirm')){
		...
		if(isset($rowBalMaster["BALANCE_DATE"]) && isset($rowBalMaster["BALANCE_DATE"]) != ""){
			...
			if($arrayPDF["RESULT"]){
				...
				$arrayResult['CONFIRM_LIST'] = $arrDetailConfirm;
	...
	<div class="wrapper-page">
		...
		<div style="margin-left:42px;">
			<table>
				...
				if(isset($arrDetail["SHR"])){
					$acceptShare = $data["ยืนยันทุรเรือนหุ้น"]??null;
		$html .='
				<tr>
					<td>ทุนเรือนหุ้น</td>
					<td class="right">'.($arrDetail["SHR"]["BALANCE_AMT"]??null).'</td>
					<td class="center"><input type="checkbox" style="margin-left:20px;"'.($acceptShare == true?"checked":null).' ></td>
					<td class="center"><input type="checkbox" style="margin-left:20px;" '.($acceptShare == true?"checked":null).'></td>
				</tr>';
				}

		foreach($arrDetail["LON"] AS $arrLoan){
			$acceptLoan = $arrLoan["ยืนยัน"]?? null;
			$html .='
			<tr>
				<td>'.($arrLoan["LOAN_DESC"]??null).'</td>
				<td class="right">'.($arrLoan["BALANCE_AMT"]??null).'</td>
				<td class="center"><input type="checkbox" style="margin-left:20px;"'.($acceptLoan == true?"checked":null).' ></td>
				<td class="center"><input type="checkbox" style="margin-left:20px;" '.($acceptLoan == true?"checked":null).'></td>
			</tr>';
		...
		foreach($arrDetail["DEP"] AS $arrDept){
			$acceptDeposit = $dositData["ยืนยัน"]??null;
			$html .='
			<tr>
				<td>'.($arrDept["DEP_DESC"]??null).'</td>
				<td class="right">'.($arrDept["BALANCE_AMT"]??null).'</td>
				<td class="center"><input type="checkbox" style="margin-left:20px;"'.($acceptDeposit == true?"checked":null).' ></td>
				<td class="center"><input type="checkbox" style="margin-left:20px;" '.($acceptDeposit == true?"checked":null).'></td>
			</tr>';
		}
