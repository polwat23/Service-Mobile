...
$getShareBF = $conmssql->prepare("SELECT (SHAREBEGIN_AMT * 10) AS SHAREBEGIN_AMT FROM shsharemaster WHERE member_no = :member_no");
...
$getOldLoanBal = $conmssql->prepare("SELECT PRINCIPAL_BALANCE FROM lncontmaster WHERE member_no = :member_no 
...
$getLoanConstant = $conmssql->prepare("SELECT RDINTDEC_TYPE,RDINTSATANG_TYPE FROM lnloanconstant");
$getLoanConstant->execute();
$rowLoanCont = $getLoanConstant->fetch(PDO::FETCH_ASSOC);
$getDataForCalInt = $conmssql->prepare("SELECT LASTCALINT_DATE,INT_CONTINTRATE,PRINCIPAL_BALANCE,LOANPAYMENT_TYPE
										FROM lncontmaster 
