...
$minsalary = $rowCredit["SALARY_AMOUNT"] * 0.3;
$moneycoop = $rowShare["PERIODSHARE_AMT"] + $rowLoan["PERIOD_PAYMENT"];
$request_amt = $rowCredit["SALARY_AMOUNT"] - ($minsalary + $moneycoop);
...
if($member_date_count > $rowCredit["STARTMEMBER_TIME"]){
	if($sharestk_amt > $maxloan_amt){
		...
		$request_amt = $rowCredit["SALARY_AMOUNT"] ;
		$loan_permit_amt = $rowCredit["MAXLOAN_AMT"];
	}
}else{
	$maxloan_amt = 0;
