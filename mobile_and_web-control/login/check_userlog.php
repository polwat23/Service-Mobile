...
if($lib->checkCompleteArgument(['pin'],$dataComing)){
	...
	if($rowResign["RESIGN_STATUS"] == '1'){
		$updateStatus = $conmssql->prepare("UPDATE gcmemberaccount SET account_status = '-6' WHERE member_no = :member_no");
	...
	if($dataComing["channel"] == "mobile_app" && isset($dataComing["is_root"])){
		if($dataComing["is_root"] == "1"){
			$insertBlackList = $conmssql->prepare("INSERT INTO gcdeviceblacklist(unique_id,member_no,type_blacklist,new_id_token,old_id_token)
			...
			$updateBlacklist = $conmssql->prepare("UPDATE gcdeviceblacklist SET is_blacklist = '0' WHERE unique_id = :unique_id and type_blacklist = '1'");
			$updateBlacklist->execute([':unique_id' => $dataComing["unique_id"]]);
		}
	}
	if(isset($dataComing["flag"]) && $dataComing["flag"] == "TOUCH_ID"){
		$is_refreshToken_arr = $auth->refresh_accesstoken($dataComing["refresh_token"],$dataComing["unique_id"],$conmssql,
	...
	$checkPinNull = $conmssql->prepare("SELECT pin,account_status FROM gcmemberaccount WHERE member_no = :member_no and account_status IN('1','-9')");
	...
	if(isset($rowPinNull["pin"])){
		...
		$updatePin = $conmssql->prepare("UPDATE gcmemberaccount SET pin = :pin WHERE member_no = :member_no");
