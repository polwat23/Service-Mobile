	...
	if(isset($rowPassword["user_type"])){
		...
		if ($valid_pass) {
			...
			try{
				...
				$getMemberLogged = $conmssql->prepare("SELECT id_token FROM gcuserlogin WHERE member_no = :member_no and channel = :channel and is_login = '1'");
				...
				if($insertToken->execute([
					...
					$id_token = $conmssql->lastInsertId();
					if(isset($dataComing["firsttime"])){
						$firstapp = 0;
					}else{
						$firstapp = 1;
					}
					$insertLogin = $conmssql->prepare("INSERT INTO gcuserlogin(member_no,device_name,channel,unique_id,status_firstapp,id_token) 
					...
					if($insertLogin->execute([
						...
						$arrPayloadNew = array();
						$arrPayloadNew['id_userlogin'] = $conmssql->lastInsertId();
						...
						if($arrPayload["PAYLOAD"]["channel"] == 'mobile_app'){
							...
							if(isset($dataComing["hms_token"]) && $dataComing["hms_token"] != ""){
								$updateFCMToken = $conmssql->prepare("UPDATE gcmemberaccount SET hms_token = :hms_token  WHERE member_no = :member_no");
								$updateFCMToken->execute([
						...
						$updateAccessToken = $conmssql->prepare("UPDATE gctoken SET access_token = :access_token WHERE id_token = :id_token");
						if($updateAccessToken->execute([
							...
							$updateWrongPassCount = $conmssql->prepare("UPDATE gcmemberaccount SET counter_wrongpass = 0  WHERE member_no = :member_no");
			...
			$updateCounter = $conmssql->prepare("UPDATE gcmemberaccount SET counter_wrongpass = counter_wrongpass + 1 WHERE member_no = :member_no");
			$updateCounter->execute([':member_no' => $member_no]);
			$getCounter = $conmssql->prepare("SELECT counter_wrongpass FROM gcmemberaccount WHERE member_no = :member_no");
			$getCounter->execute([':member_no' => $member_no]);
			$rowCounter = $getCounter->fetch(PDO::FETCH_ASSOC);
			if($rowCounter["counter_wrongpass"] >= 5){
				$updateAccountStatus = $conmssql->prepare("UPDATE gcmemberaccount SET prev_acc_status = account_status,account_status = '-8',counter_wrongpass = 0 WHERE member_no = :member_no");
