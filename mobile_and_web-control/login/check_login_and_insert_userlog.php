...
if($lib->checkCompleteArgument(['member_no','api_token','password','unique_id'],$dataComing)){
	...
	if(strlen($dataComing["password"]) === 13){
		$checkMember = $conoracle->prepare("SELECT card_person,resign_status,mem_telmobile FROM mbmembmaster WHERE member_no = :member_no and card_person = :password");
		$checkMember->execute([
			':member_no' => $member_no,
		...
		if(isset($rowMemberChecked["CARD_PERSON"])){
			if($rowMemberChecked["RESIGN_STATUS"] == 0){
