...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanStatement')){
		...
		$date_now = date('Y-m-d');
		$fetchLastStmAcc = $conoracle->prepare("SELECT * FROM (SELECT lnm.loancontract_no from lncontmaster lnm 
							LEFT JOIN lncontstatement lns ON lnm.loancontract_no = lns.loancontract_no
							WHERE lnm.member_no = :member_no and lnm.contract_status = 1 and 
							entry_date IS NOT NULL ORDER BY entry_date DESC) WHERE rownum <= 1");
