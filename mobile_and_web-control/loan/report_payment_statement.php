...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanInfo')){
		...
		$payoutslip_no = $dataComing["payoutslip_no"];
		...
		if(sizeof($arrDetail) > 0 ){
			$arrayPDF = GeneratePdfDoc($arrDetail,$sum_payout_amt,$sum_payoutclr_amt,$sum_payoutnet_amt,$lib,$count,$date,$payload['member_no']);
			if($arrayPDF["RESULT"]){
...
function GeneratePdfDoc($arrDetail,$sum_payout_amt,$sum_payoutclr_amt,$sum_payoutnet_amt,$lib,$count,$date,$member_no){
	$html = '<style>
	@font-face {
	...
	* {
	  ...
	  <div style="width: 100%;" class="sub-table">
		<div style="border-bottom: 1px solid #3c3939de;border-top: 1px solid #3c3939de;">
		  ...
		  <div>
			...
			<div style="display:inline;font-weight:bold; padding-left:123px;">สาขา</div>
	  ...
	  <div style="display:flex;height: 30px;padding:0px">
		...
		foreach ($dataArr["RECEIVE"] as $Listdel) {
			if($Listdel["SLIPITEMTYPE_CODE"] == "LON"){
				...
				$html .= '
				 <div style="display:flex;height: 30px;padding:0px">
					<div style="margin-left:100px;   text-decoration: underline;"></div>
		...
		foreach($dataArr["PAY"] as $lisData) {
			if($lisData["MONEYTYPE_CODE"] == "CBT"){	
				...
				<div style="margin-left:395px;   width:220px; ">' . ($lisData["BRANCH_NAME"] ?? null) . '</div>
				<div  style=" width:100px; margin-left:610px;">' . ($lisData["EXPENSE_ACCID"] ?? null) . '</div> 
				<div  style=" width:115px; margin-left:800px; text-align:right;">' . (number_format($lisData["EXPENSE_AMT"],2) ?? null) . '</div>
				...
				$html .= '
				<div style="display:flex;height: 30px;padding:0px">
				<div style="margin-left:30px;   text-decoration: underline;">จ่าย</div>
				<div style="margin-left:70px;">' . ($lisData["MONEYTYPE_DESC"] ?? null) . '</div>
				<div style="margin-left:237px;"></div>
				<div style="margin-left:407px;   width:220px; "></div>
	...
	$html .= '
	  <div style=" border-top:1px dashed; border-bottom:1px dashed; height:30px; ">
		 <div style="display:flex; font-weight:bold;" class="nowrap" >
			  ...
			  <div style="margin-left:287px;  width:50px;">งวด</div>
	...
	$pathfile_show = '/resource/pdf/payment/'.$member_no.'.pdf?v='.time();
	$arrayPDF = array();
