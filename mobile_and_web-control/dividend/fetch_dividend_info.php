...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DividendInfo')){
		...
		while($rowYear = $getYeardividend->fetch(PDO::FETCH_ASSOC)){
			...
			$getPaydiv = $conoracle->prepare("SELECT yucf.methpaytype_desc AS TYPE_DESC,ymp.pay_amt as pay_amt
			...
			if($receiveMinus < 0){
				...
				$arrayRecv["BANK"] = '*** กรณียอดหักไม่พอจ่าย กรุณามาชำระได้ที่สหกรณ์ฯ';
