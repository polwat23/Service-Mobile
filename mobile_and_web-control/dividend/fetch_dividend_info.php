...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'DividendInfo')){
		...
		while($rowYear = $getYeardividend->fetch(PDO::FETCH_ASSOC)){
			$arrDividend = array();
			$getDivMaster = $conoracle->prepare("SELECT div_amt,avg_amt,AVGSHOP_PAYMENT FROM yrdivmaster WHERE member_no = :member_no and div_year = :div_year");
			...
			$arrDividend["AVGSHOP_AMT"] = number_format($rowDiv["AVGSHOP_PAYMENT"],2);
			$arrDividend["SUM_AMT"] = number_format($rowDiv["DIV_AMT"] + $rowDiv["AVG_AMT"] + $rowDiv["AVGSHOP_PAYMENT"],2);
			...
			while($rowMethpay = $getMethpay->fetch(PDO::FETCH_ASSOC)){
				...
				$arrayRecv["RECEIVE_AMT"] = number_format($rowMethpay["RECEIVE_AMT"] + $rowDiv["AVGSHOP_PAYMENT"],2);
