		...
		while($rowYear = $getYeardividend->fetch(PDO::FETCH_ASSOC)){
			...
			$getMethpay = $conoracle->prepare("SELECT
												...
												FROM 
													...
													LEFT JOIN yrucfmethpay ycf ON ym.METHPAYTYPE_CODE = ycf.METHPAYTYPE_CODE
												WHERE
													YM.MEMBER_NO = :member_no
													AND YM.METHPAYTYPE_CODE IN('CBT','CSH','DEP','SAL','CHQ','NXT')
			...
			while($rowMethpay = $getMethpay->fetch(PDO::FETCH_ASSOC)){
				...
				$arrayRecv["RECEIVE_DESC"] = $rowMethpay["METHPAYTYPE_DESC"];
			...
			$getPaydiv = $conoracle->prepare("SELECT yucf.methpaytype_desc AS TYPE_DESC,ymp.expense_amt as pay_amt
											...
											WHERE ymp.MEMBER_NO = :member_no and ymp.div_year = :div_year and ymp.methpaytype_code NOT IN('CBT','CSH','DEP','SAL','CHQ','NXT')");
			$getPaydiv->execute([
				':member_no' => $member_no,
				':div_year' => $rowYear["DIV_YEAR"]
			]);
