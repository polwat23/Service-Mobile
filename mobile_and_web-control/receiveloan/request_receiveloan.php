...
if($lib->checkCompleteArgument(['menu_component','contract_no','deptaccount_no','amt_transfer'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanReceive')){
		...
		if($slipPayout["RESULT"]){
			$receiveLon = $cal_loan->receiveLoanOD($conmssql,$config,$contract_no,$dataCont,null,$dataComing["amt_transfer"],
			$payoutslip_no,$ref_no,$deptaccount_no,0,$payload,$dataComing["app_version"],$dateOper,$log);
			if($receiveLon["RESULT"]){
				$depositMoney = $cal_dep->DepositMoneyInside($conmssql,$deptaccount_no,$destvcid["ACCOUNT_ID"],$itemtypeDeposit,
					...
					foreach($arrToken["LIST_SEND"] as $dest){
						if($dest["RECEIVE_NOTIFY_TRANSACTION"] == '1'){
							...
							if($lib->sendNotify($arrPayloadNotify,"person")){
								...
								$updateSyncNoti = $conmssql->prepare("UPDATE dpdeptstatement SET sync_notify_flag = '1' WHERE deptslip_no = :ref_slipno and deptaccount_no = :deptaccount_no");
