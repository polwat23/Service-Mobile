...
if($lib->checkCompleteArgument(['menu_component','contract_no','deptaccount_no','amt_transfer'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'LoanReceive')){
		...
		$insertReceiveloanOd = $conmysql->prepare("INSERT INTO gcreceiveloanod(member_no, loancontract_no, amount_receive, withdrawable_amt,principal_balance,deptaccount_no, calint_from, calint_to,id_userlogin,payload)
													VALUES(:member_no, :contract_no ,:amount_receive,:withdrawable_amt,:principal_balance,:deptaccount_no,:calint_from, NOW(),:id_userlogin ,:payload)");
