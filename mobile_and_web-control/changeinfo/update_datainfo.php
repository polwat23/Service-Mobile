		...
		if(isset($dataComing["email"]) && $dataComing["email"] != ""){
			...
			if($updateEmail->execute([
				...
				if($arrConstInfo["email"] == '1'){
					$updateEmailOracle = $conmssql->prepare("UPDATE mbmembmaster SET addr_email = :email WHERE member_no = :member_no");
		...
		if(isset($dataComing["tel"]) && $dataComing["tel"] != ""){
			...
			if($updateTel->execute([
				...
				if($arrConstInfo["tel"] == '1'){
					$updateTelOracle = $conmssql->prepare("UPDATE mbmembmaster SET addr_mobilephone = :phone_number WHERE member_no = :member_no");
