...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SettingMemberInfo')){
		...
		$arrayConst = array();
		$arrayDataGrp = array();
		$getConstChangeInfo = $conoracle->prepare("SELECT const_code,is_change FROM gcconstantchangeinfo");
		...
		if($arrayConst["email"] == '1'){
			$getEmail = $conoracle->prepare("SELECT email FROM gcmemberaccount WHERE member_no = :member_no");
			$getEmail->execute([':member_no' => $payload["member_no"]]);
			$rowEmail = $getEmail->fetch(PDO::FETCH_ASSOC);
			$arrayDataGrp["EMAIL"] = $rowEmail["EMAIL"];
		}
		if($arrayConst["tel"] == '1'){
			$getPhone = $conoracle->prepare("SELECT phone_number FROM gcmemberaccount WHERE member_no = :member_no");
