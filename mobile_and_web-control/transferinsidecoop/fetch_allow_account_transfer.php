...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	...
	$func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferSelfDepInsideCoop')){
		...
		while($rowDataAccAll = $getAllAcc->fetch(PDO::FETCH_ASSOC)){
			$fetchConstantAllowDept = $conoracle->prepare("SELECT allow_deposit_inside,allow_withdraw_inside FROM gcconstantaccountdept 
		...
		if($dataComing["menu_component"] == 'TransferDepInsideCoop'){
			$getAccFav = $conoracle->prepare("SELECT fav_refno,name_fav,destination FROM gcfavoritelist WHERE member_no = :member_no and flag_trans = 'TRN'");
