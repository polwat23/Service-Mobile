...
if($lib->checkCompleteArgument(['menu_component','deptaccount_no','amt_transfer'],$dataComing)){
	...
	$func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferSelfDepInsideCoop')){
		...
		$dateTrans = date('c');
		$penalty_amt = 0;
		$checkStatusAcc = $conmssql->prepare("SELECT DPM.DEPTCLOSE_STATUS,DPT.DEPTGROUP_CODE FROM DPDEPTMASTER DPM 
		...
		$getTypeAcc = $conmssql->prepare("SELECT DEPTTYPE_CODE FROM dpdeptmaster WHERE deptaccount_no = :acc_no");
		...
		if($rowContDeptType["IS_CHECK_PENALTY"] == '1'){
			...
			if(substr($rowDepPay["GRP_ITEMTYPE"],0,1) == 'W'){
				$checkItemIsCount = $conmssql->prepare("SELECT COUNT(*) as IS_NOTCOUNT FROM dpucfwithncount 
		...
		if($count_trans > $rowContDeptType["MAXWITHD_INMONTH"]){
			$getContDeptTypeFee = $conmssql->prepare("SELECT CHARGE_FLAG,s_chrg_amt1 as MIN_FEE,s_chrg_perc1 as PERCENT_FEE,s_chrg_amt2 as MAX_FEE 
