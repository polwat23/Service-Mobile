...
if($lib->checkCompleteArgument(['menu_component','source_deptaccount_no','deptaccount_no'],$dataComing)){
	...
	$func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferSelfDepInsideCoop')){
		...
		$arrarDataAcc = array();
		$getDataAcc = $conoracle->prepare("SELECT dpm.deptaccount_name,dpm.DEPT_OBJECTIVE,dpt.depttype_desc,dpm.depttype_code
		...
		if(isset($rowDataAcc["DEPTTYPE_DESC"])){
			$fetchConstantAllowDept = $conoracle->prepare("SELECT allow_deposit_inside FROM gcconstantaccountdept 
			...
			if($rowContAllow["ALLOW_DEPOSIT_INSIDE"] == '1'){
				...
				if($checkSeqAmt["RESULT"]){
					if($checkSeqAmt["CAN_DEPOSIT"]){
						...
						$arrarDataAcc["ACCOUNT_NAME"] = preg_replace('/\"/','',$rowDataAcc["DEPTACCOUNT_NAME"].' '.$rowDataAcc["DEPT_OBJECTIVE"]);
