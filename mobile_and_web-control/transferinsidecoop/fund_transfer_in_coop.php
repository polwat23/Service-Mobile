...
if($lib->checkCompleteArgument(['menu_component','from_deptaccount_no','to_deptaccount_no','amt_transfer','penalty_amt'],$dataComing)){
	...
	$func->check_permission($payload["user_type"],$dataComing["menu_component"],'TransferSelfDepInsideCoop')){
		...
		$dateOper = date('c');
		$checkStatusAcc = $conmssql->prepare("SELECT DPM.DEPTCLOSE_STATUS,DPT.DEPTGROUP_CODE
		...
		// Start-Withdraw
		$getAccDataSrc = $conmssql->prepare("SELECT DPT.ACCOUNT_ID,DPM.WITHDRAW_COUNT,DPM.PRNCBAL,DPT.MINPRNCBAL,
		...
		$getDepPaytype = $conmssql->prepare("SELECT group_itemtpe as GRP_ITEMTYPE,MONEYTYPE_SUPPORT 
		...
		$getAccDataDest = $conmssql->prepare("SELECT DPT.ACCOUNT_ID,DPM.WITHDRAW_COUNT,DPM.PRNCBAL,
		...
		$getMapAccidSrc = $conmssql->prepare("SELECT ACCOUNT_ID FROM VCMAPACCID WHERE SYSTEM_CODE = 'DEP' AND SLIPITEMTYPE_CODE = 'DEP' AND SHRLONTYPE_CODE = :depttype_code");
		$getMapAccidSrc->execute([':depttype_code' => $rowAccData["DEPTTYPE_CODE"]]);
		$rowMapAccSrc = $getMapAccidSrc->fetch(PDO::FETCH_ASSOC);
		$getMapAccidDest = $conmssql->prepare("SELECT ACCOUNT_ID FROM VCMAPACCID WHERE SYSTEM_CODE = 'DEP' AND SLIPITEMTYPE_CODE = 'DEP' AND SHRLONTYPE_CODE = :depttype_code");
		...
		$getMaxSeqNo = $conmssql->prepare("SELECT MAX(SEQ_NO) as MAX_SEQ_NO FROM dpdeptstatement WHERE deptaccount_no = :deptaccount_no");
		$getMaxSeqNo->execute([':deptaccount_no' => $from_account_no]);
		$rowMaxSeqNo = $getMaxSeqNo->fetch(PDO::FETCH_ASSOC);
		$getLastDpSlipNo = $conmssql->prepare("SELECT DOCUMENT_FORMAT,DOCUMENT_PREFIX,LAST_DOCUMENTNO,DOCUMENT_YEAR,DOCUMENT_LENGTH
		...
		if($insertDpSlip->execute($arrExecute)){
			...
			$insertStatement = $conmssql->prepare("INSERT INTO DPDEPTSTATEMENT(COOP_ID,DEPTACCOUNT_NO,SEQ_NO,DEPTITEMTYPE_CODE,OPERATE_DATE,DEPTITEM_AMT,BALANCE_FORWARD,PRNCBAL,ENTRY_ID,ENTRY_DATE,
													...
													VALUES(:coop_id,:from_account_no,:seq_no,:itemtype_code,GETDATE(),:slip_amt,:balance_forward,:after_trans_amt,'MOBILE',CONVERT(DATETIME,:entry_date),
													CONVERT(DATETIME,:lastcalint_date),GETDATE(),:cash_type,CONVERT(DATETIME,:entry_date),:deptslip_no,'1')");
			if($insertStatement->execute($arrExecuteStm)){
				// Start-Penalty-Cal
				if($dataComing["penalty_amt"] > 0){
					$getMapAccidFee = $conmssql->prepare("SELECT ACCOUNT_ID FROM VCMAPACCID WHERE SYSTEM_CODE = 'DEP' AND SLIPITEMTYPE_CODE = 'FEE' AND SHRLONTYPE_CODE = '00'");
					...
					$insertDpSlipPenalty = $conmssql->prepare("INSERT INTO DPDEPTSLIP(DEPTSLIP_NO,COOP_ID,DEPTACCOUNT_NO,DEPTTYPE_CODE,   
														...
														POSTTOVC_FLAG,TAX_AMT,INT_BFYEAR,ACCID_FLAG,SHOWFOR_DEPT,GENVC_FLAG,PEROID_DEPT,CHECKCLEAR_STATUS,   
														TELLER_FLAG,OPERATE_TIME) 
														VALUES(:deptslip_no,:coop_id,:deptaccount_no,:depttype_code,:coop_id,:deptgrp_code,GETDATE(),:itemtype_code,
														:slip_amt,:cash_type,:prncbal,:withdrawable_amt,:checkpend_amt,'MOBILE',CONVERT(DATETIME,:entry_date),:laststmno,:itemtype_code,
														CONVERT(DATETIME,:lastcalint_date),GETDATE(),1,0,0,0,:acc_id,2,:refer_deptslip_no,0,0,:slip_amt,'DEP',0,0,0,1,1,1,0,1,1,
														CONVERT(DATETIME,:entry_date))");
				...
				$updateDeptMaster = $conmssql->prepare("UPDATE DPDEPTMASTER SET withdrawable_amt = :withdraw_after_pay,prncbal = :prncbal_after_pay,
														lastmovement_date = CONVERT(DATETIME,:entry_date),
														lastaccess_date = CONVERT(DATETIME,:entry_date),laststmseq_no = :seq_no
				...
				if($updateDeptMaster->execute($arrUpdateMaster)){
					...
					$getDepPaytypeDest = $conmssql->prepare("SELECT group_itemtpe as GRP_ITEMTYPE,MONEYTYPE_SUPPORT 
														FROM dpucfrecppaytype WHERE recppaytype_code = :itemtype");
					$getDepPaytypeDest->execute([':itemtype' => $itemtypeDepositDest]);
					$rowDepPayDest = $getDepPaytypeDest->fetch(PDO::FETCH_ASSOC);
					$getMaxSeqNoDest = $conmssql->prepare("SELECT MAX(SEQ_NO) as MAX_SEQ_NO FROM dpdeptstatement WHERE deptaccount_no = :deptaccount_no");
					...
					$insertDpSlipDest = $conmssql->prepare("INSERT INTO DPDEPTSLIP(DEPTSLIP_NO,COOP_ID,DEPTACCOUNT_NO,DEPTTYPE_CODE,   
														...
														VALUES(:deptslip_no,:coop_id,:deptaccount_no,:depttype_code,:coop_id,:deptgrp_code,GETDATE(),:itemtype_code,
														:slip_amt,:cash_type,:prncbal,:withdrawable_amt,:checkpend_amt,'MOBILE',CONVERT(DATETIME,:entry_date),:laststmno,:itemtype_code,
														CONVERT(DATETIME,:lastcalint_date),GETDATE(),1,0,0,0,:acc_id,1,0,0,
														:slip_amt,0,0,0,1,1,1,0,1,1,CONVERT(DATETIME,:entry_date))");
					if($insertDpSlipDest->execute($arrExecuteDest)){
						...
						$insertStatementDest = $conmssql->prepare("INSERT INTO DPDEPTSTATEMENT(COOP_ID,DEPTACCOUNT_NO,SEQ_NO,DEPTITEMTYPE_CODE,OPERATE_DATE,DEPTITEM_AMT,BALANCE_FORWARD,PRNCBAL,ENTRY_ID,ENTRY_DATE,
																CALINT_FROM,CALINT_TO,CASH_TYPE,OPERATE_TIME,DEPTSLIP_NO,SYNC_NOTIFY_FLAG)
																VALUES(:coop_id,:to_account_no,:seq_no,:itemtype_code,GETDATE(),:slip_amt,:balance_forward,:after_trans_amt,'MOBILE',CONVERT(DATETIME,:entry_date),
																CONVERT(DATETIME,:lastcalint_date),GETDATE(),:cash_type,CONVERT(DATETIME,:entry_date),:deptslip_no,'1')");
						if($insertStatementDest->execute($arrExecuteStmDest)){
							...
							$updateDeptMasterDest = $conmssql->prepare("UPDATE DPDEPTMASTER SET withdrawable_amt = :withdraw_after_pay,prncbal = :prncbal_after_pay,
																	lastmovement_date = CONVERT(DATETIME,:entry_date),
																	lastaccess_date = CONVERT(DATETIME,:entry_date),laststmseq_no = :seq_no
