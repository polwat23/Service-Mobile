	...
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'CremationInfo')){
		...
		$getDataWc = $conoracle->prepare("select wcdeptmaster.coop_id,
                                 (case
                                    ...
                                    when wcdeptmaster.coop_id = '041001' then 'สอ.'
		...
		while($rowDataWc = $getDataWc->fetch(PDO::FETCH_ASSOC)){
			if(isset($rowDataWc["DEPTACCOUNT_NO"]) && $rowDataWc["DEPTACCOUNT_NO"] != ""){
				...
				if($rowDataWc["COOP_ID"] == '010000'){
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no and coop_id = :coop_id  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"]),
						':coop_id' => $rowDataWc["COOP_ID"]
					]);
				}else if($rowDataWc["COOP_ID"] == '080000'){
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no and coop_id = :coop_id  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"]),
						':coop_id' => $rowDataWc["COOP_ID"]
					]);
				}else if($rowDataWc["COOP_ID"] == '090000'){
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no and coop_id = :coop_id  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"]),
						':coop_id' => $rowDataWc["COOP_ID"]
					]);
				}else if($rowDataWc["COOP_ID"] == '041001'){
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no and coop_id = :coop_id  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"]),
						':coop_id' => $rowDataWc["COOP_ID"]
					]);
				}else if($rowDataWc["COOP_ID"] == '020000'){
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no and coop_id = :coop_id  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"]),
						':coop_id' => $rowDataWc["COOP_ID"]
					...
					$getPersonAccountWC = $conwc->prepare("select seq_no,transferee_name from wccodeposit where deptaccount_no = :account_no  order by seq_no ");
					$getPersonAccountWC->execute([
						':account_no' => TRIM($rowDataWc["DEPTACCOUNT_NO"])
					]);
				}
				...
				//$arrayDataWc["BENEFICIARY_LIST"] = array();
