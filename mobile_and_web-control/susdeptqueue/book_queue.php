...
if($lib->checkCompleteArgument(['menu_component','queue_id'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SuspendingDebtQueue')){
		...
		if($fetchBranch->rowCount() > 0){
			...
			$insertConst = $conmysql->prepare("INSERT INTO gcsusdebtqueuedetail (queue_id, member_no) VALUES (:queue_id, :member_no)");
			if($insertConst->execute([
				...
				$logStruc = [
					":error_menu" => $filename,
					":error_code" => "WS1038",
					":error_desc" => "ไม่สามารถ Insert ลง gcsusdebtqueuedetail ได้ "."\n".$insertConst->queryString."\n".json_encode([
						':queue_id' => $dataComing["queue_id"],
				...
				$log->writeLog('errorusage',$logStruc);
				$message_error = "ไม่สามารถ Insert ลง gcsusdebtqueuedetail ได้ "."\n".$insertConst->queryString."\n".json_encode([
					':queue_id' => $dataComing["queue_id"],
					':member_no' => $dataComing["member_no"]
				]);
				$lib->sendLineNotify($message_error);
