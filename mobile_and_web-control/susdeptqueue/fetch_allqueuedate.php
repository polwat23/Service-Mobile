...
if($lib->checkCompleteArgument(['menu_component'],$dataComing)){
	if($func->check_permission($payload["user_type"],$dataComing["menu_component"],'SuspendingDebtQueue')){
		$arrayGroup = array();
		$fetchBranch = $conmysql->prepare("SELECT queue_date,coop_branch_id,SUM(max_queue) as max_queue,SUM(remain_queue) as remain_queue FROM gcsusdebtqueuemaster WHERE queue_status = '1' AND coop_branch_id = :coop_branch_id AND
