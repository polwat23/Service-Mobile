...
&& isset($dataComing["user_type"]) && isset($dataComing["menu_component"])){
	...
	if($func->check_permission($dataComing["user_type"],$dataComing["menu_component"],$conmysql,'MemberInfo')){
		...
		$memberInfoMobile = $conmysql->prepare("SELECT phone_number,email,path_avatar,member_no FROM mdbmemberaccount WHERE member_no = :member_no");
