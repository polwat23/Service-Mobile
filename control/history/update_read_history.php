...
if(isset($dataComing["access_token"]) && isset($dataComing["unique_id"]) && isset($dataComing["member_no"]) && isset($dataComing["id_history"])
&& isset($dataComing["user_type"]) && isset($dataComing["menu_component"]) && isset($dataComing["refresh_token"])){
	...
	if($func->check_permission($dataComing["user_type"],$dataComing["menu_component"],$conmysql,'Notification')){
		$readHistory = $conmysql->prepare("UPDATE mdbhistory SET his_read_status = '1' WHERE member_no = :member_no and id_history = :id_history");
