...
class libraryLine {
	...
	public function getLineIdNotify($member_no=null) {
		$getToken = $this->con->prepare("SELECT line_token FROM gcmemberaccount WHERE member_no = :member_no");
		$getToken->execute([':member_no' => $member_no]);
		if($getToken->rowCount() > 0){
			$rowLimit = $getToken->fetch(\PDO::FETCH_ASSOC);
			$getNotifyData = $this->con->prepare("SELECT line_token FROM lbnotify WHERE member_no = :member_no AND is_notify = '0'");
			$getNotifyData->execute([':member_no' => $member_no]);
			if($getNotifyData->rowCount() > 0){
				return false;
			}else{
	...
	public function notBindAccount(){
		...
		$dataContent["body"]["contents"][0]["color"] = "#0679b2";
		...
		$dataContent["body"]["contents"][2]["color"] = "#0679b2";
		return $dataContent;
	}
