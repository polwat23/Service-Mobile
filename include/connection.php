...
class connection {
	...
	public function connecttooracle() {
		...
		$json_data = json_decode($json,true);
		try{
			$dbuser = $json_data["DBORACLE_USERNAME_BACKUP"];
			$dbpass = $json_data["DBORACLE_PASSWORD_BACKUP"];
			$dbname = "(DESCRIPTION =
						(ADDRESS_LIST =
						  (ADDRESS = (PROTOCOL = TCP)(HOST = ".$json_data["DBORACLE_HOST_BACKUP"].")(PORT = 1521))
						)
						(CONNECT_DATA =
						  (SERVICE_NAME = ".$json_data["DBORACLE_SERVICE_BACKUP"].")
						)
					  )";
